...
function createFoundation(initial: FormState = { values: {}, errors: {}, touched: {} }) {
  let state = initial;
...
describe('FormFoundation', () => {
  ...
  it('registerField 不覆盖已存在的值', () => {
    const { foundation, getState } = createFoundation({ values: { username: 'existing' }, errors: {}, touched: {} });
    foundation.registerField('username', {}, 'semi');
  ...
  it('validateField：required 规则，有值时通过', async () => {
    const { foundation, getState } = createFoundation({ values: { username: 'semi' }, errors: {}, touched: {} });
    foundation.registerField('username', { rules: [{ required: true }] });
    const error = await foundation.validateField('username');
  ...
  it('validateField：pattern 规则不匹配时返回错误', async () => {
    const { foundation } = createFoundation({ values: { email: 'not-an-email' }, errors: {}, touched: {} });
  ...
  it('validateField：min/max 规则', async () => {
    const { foundation } = createFoundation({ values: { age: 3 }, errors: {}, touched: {} });
  ...
  it('validateField：自定义同步 validator', async () => {
    const { foundation } = createFoundation({ values: { age: 10 }, errors: {}, touched: {} });
  ...
  it('validateField：自定义异步 validator', async () => {
    const { foundation } = createFoundation({ values: { username: 'taken' }, errors: {}, touched: {} });
  ...
  it('validateField：数组规则按顺序执行，第一条不通过即返回', async () => {
    const { foundation } = createFoundation({ values: { username: '' }, errors: {}, touched: {} });
    foundation.registerField('username', {
      rules: [
  ...
  it('validateAll：汇总所有字段的错误', async () => {
    const { foundation } = createFoundation({ values: { username: '', age: 3 }, errors: {}, touched: {} });
    foundation.registerField('username', { rules: [{ required: true, message: '必填' }] });
    foundation.registerField('age', { rules: [{ min: 18, message: '未成年' }] });
    const errors = await foundation.validateAll();
  ...
  it('submit：校验通过时调用 onSubmit，不调用 onSubmitFail', async () => {
    const { foundation } = createFoundation({ values: { username: 'semi' }, errors: {}, touched: {} });
    foundation.registerField('username', { rules: [{ required: true }] });
