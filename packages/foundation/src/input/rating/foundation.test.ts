...
describe('handleClick', () => {
  it('点击新值时提交并清空 clearedValue', () => {
    ...
    const result = foundation.handleClick(3, 0.9);
  ...
  it('allowClear=true 时点击当前已选值清零', () => {
    ...
    const result = foundation.handleClick(2, 0.9);
...
describe('handleKeyDown', () => {
  it('ArrowRight/ArrowUp 按 step 递增', () => {
    ...
    foundation.handleKeyDown('ArrowRight');
    expect(getState().value).toBe(3);
  });

  it('ArrowLeft/ArrowDown 按 step 递减', () => {
    const { foundation, getState } = makeFoundation({ value: 2 }, { allowHalf: false });
    foundation.handleKeyDown('ArrowLeft');
  ...
  it('非方向键返回 null', () => {
    ...
    expect(foundation.handleKeyDown('Enter')).toBeNull();
  });
