...
`type="nav"` 是极简的导航条形态（对齐 Semi NavStep）：只渲染 `title`，不支持 `icon`/`description`/`status`，项间用分隔图标区分（最后一项不显示）。适合用作页面顶部的分段导航，而非流程进度展示。

```tsrx demo
../../src/demos/navigation/steps/nav.tsrx
```
...
| direction | 排列方向（`type="nav"` 时不生效） | `horizontal` \| `vertical` | `horizontal` |
| hasLine | 是否显示步骤间的连接线（`type="nav"` 时不生效） | boolean | `true` |
| initial | 起始步骤序号（影响步骤编号显示，不影响 `current`） | number | `0` |
| items | 步骤项数组 | `StepItemInput[]` | 必填 |
| size | 尺寸 | `default` \| `small` | `default` |
| status | 当前步骤的状态（`type="nav"` 时不生效，nav 不使用 status 概念） | `wait` \| `process` \| `finish` \| `error` \| `warning` | `process` |
| style | 自定义样式 | object | - |
| type | 视觉类型 | `basic` \| `fill` \| `nav` | `basic` |
| onChange | 步骤变化时的回调 | `(current: number) => void` | - |

`StepItemInput` 结构：`{ title?, description?, icon?, status?, disabled? }`——单个步骤项的 `status` 会覆盖组件级 `status` 对该步骤的自动计算。`type="nav"` 时只使用 `title` 字段，`description`/`icon`/`status` 均被忽略（对齐 Semi NavStep 本身不支持这些字段）。

## Accessibility

- 各步骤项当前依赖视觉图标（圆点/勾选/叹号/序号）区分状态，无原生语义角色；建议配合有意义的 `title`/`description` 文案，不要仅靠颜色传达状态。
- `type="nav"` 的每个步骤项携带 `role="button"`、`tabIndex`、`aria-current="step"`，支持键盘 Tab 聚焦与 Enter 触发。
