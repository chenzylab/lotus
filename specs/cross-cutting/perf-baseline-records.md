...
## Table — 2026-08-30 (commit 2966d0a)

环境：MacBook Pro (arm64, macOS 26.5.1) / Playwright bundled Chromium 1.62.1 / production build（`vite build` + `vite preview --port 4321`，非 dev server）

数据规模：1 万行（`tableVirtualizeData`，playground 既有 demo），`scroll={{ y: 240 }}` + `virtualize={{ itemSize: 44 }}` 开启固定行高虚拟滚动（复用 `base/virtual-list.ts`，与 Select/Cascader/TreeSelect 同一套算法），列定义含年龄列 `sorter` 与城市列 `filters`（3 个枚举值循环填充，筛选"北京"命中约 3333 行）。**架构现状**：Table 组件的虚拟化是可选 opt-in（`virtualize` prop 需要同时声明 `scroll.y` 才生效，不像 Select 系列默认建议开启），本次测量的是"用户已正确开启虚拟化"这条路径；未开启虚拟化时的 1 万行渲染不在本次测量范围内（不属于本组件推荐的使用方式，类比 Select 系列同样要求大数据量下开启 `virtualize`）。

测量方法：`performance.now()` 记录「虚拟滚动」（`scrollTop` 赋值触发虚拟区间重算到两帧绘制完成，6 次来回滚动）、「排序」（点击年龄列表头触发全量 1 万行排序 + 虚拟区间重算，5 次连续切换升降序）、「筛选」（点击筛选图标 → 勾选"北京" → 确定，触发全量筛选 + 虚拟区间重算，4 次循环）三个场景。

| 操作 | 耗时 | 判定 |
|---|---|---|
| 虚拟滚动（6 次） | 30.1~39.8ms，均值 37.8ms | 合格 |
| 排序切换（5 次） | 31.1~81.6ms，均值 47.9ms | 合格 |
| 筛选确认（4 次） | 63.7~81.1ms，均值 70.8ms | 合格 |

判定结论：**达标，远低于 200ms 合格线，无需优化**。排序/筛选均为全量重算（1 万行整体排序/过滤后重新计算虚拟区间），不是增量算法，但在当前数据规模下开销可接受；筛选耗时略高于排序和滚动（均值 70.8ms vs 47.9ms/37.8ms），推测是筛选路径多了下拉面板开关的 DOM 结构变化，仍在合格范围内不需要专门优化。若后续数据规模显著超出 1 万行（如 10 万+）应重新复测，全量重算的排序/筛选算法可能在更大规模下暴露瓶颈。

## Toast / Notification — 2026-08-30 (commit 2966d0a)

环境：MacBook Pro (arm64, macOS 26.5.1) / Playwright bundled Chromium 1.62.1 / production build（`vite build` + `vite preview --port 4321`，非 dev server）

数据规模：单例懒挂载容器模式（`ensureMounted()` 首次调用创建挂载点，后续复用），高频调用场景关切的是"重复触发+销毁循环"是否随调用次数产生性能退化，而非单次调用的绝对耗时。

测量方法：连续 200 次"触发单条 Toast/Notification → 两帧绘制完成 → destroyAll()"循环（`performance.now()` 测量触发到两帧绘制完成的耗时），对比前 10 次均值与后 10 次均值判断是否有退化趋势，同时记录 200 次里的最大值。

| 组件 | 前 10 次均值 | 后 10 次均值 | 200 次最大值 | 判定 |
|---|---|---|---|---|
| Toast | 66.0ms | 59.9ms | 87.4ms | 合格，无退化 |
| Notification | 67.3ms | 60.8ms | 95.3ms | 合格，无退化 |

判定结论：**两者均达标，200 次循环后耗时反而略低于最初 10 次（属于正常波动，不是负退化的证据），无内存/性能随调用次数劣化的迹象**，与之前已验证的"100 次挂载/卸载无残留 DOM"功能测试结论一致（单例懒挂载容器设计确实避免了重复创建/销毁容器的开销）。均远低于 200ms 合格线，无需优化。
