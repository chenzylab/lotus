...
- **Tag 装饰色（16 色）暗色模式下 `yellow`（1.34）/`amber`（1.56）白字对比度极低**（`.lotus-tag-colorful.lotus-tag-solid` 统一用白字配 `--lotus-color-tag-*`）。用 Semi 一手来源暗色模式真实数值（`_palette.scss` 第366/376行 `--semi-yellow-5`/`--semi-amber-5`）独立计算得 1.337/1.562，与 lotus 实测几乎完全一致；核对 `semi-foundation/tag/mixin.scss` 确认 Semi 的 solid 类型对全部 16 色无条件统一用白字，没有为过亮颜色单独切换黑字——这是 Semi 官方本身的固有权衡，不是 lotus 引入的额外劣化，同样不应被"修复"成给这两色单独换黑字（除非未来明确要做"lotus 独立于 Semi 演进"的改进，而非当前"对齐一手来源"阶段的范畴）。
- **FloatButton/Button/Tag 的 AI 渐变按钮（`colorful` prop）default 态白字对比度最低点仅 3.11:1**（渐变起点 `#e945ff` 粉紫色与白字），低于正文 4.5:1 但满足 UI 组件 3.0:1 门槛；hover 态 4.35:1、active 态 6.15:1 均达标。这个数值来自 2026-08-29 核实一手来源（`~/i/semi-design/packages/semi-theme-default/scss/_palette.scss` 的 `--semi-ai-general-5/6/7`）修正 `aiColor` 后的真实计算结果——修正前 lotus 用的是拍脑袋近似色（角度/色标数/顺序均不对齐一手来源），错误实现下 default 态对比度实测只有 2.83:1（更低），侧面印证"先核对一手来源再判断是否权衡"比"先假设当前实现正确"更可靠。"多彩渐变+统一白字"这类设计范式下渐变最亮端对比度偏低是 Semi 官方一手来源本身的固有权衡（Semi 自己也没有为渐变按钮改变白字颜色），不是 lotus 引入的缺陷，同样不应被"修复"成改小渐变亮度或换用深色文字。**补充（新增 `ai-color-contrast.test.ts` 固化为自动化测试时发现）**：暗色模式下这个权衡更极端——`generalActive` 白字对比度低至 1.57~1.62（仍高于"完全不可辨识"的 1.5 下限，能通过测试但已经很接近）。用 Semi 一手来源暗色模式数值（`_palette.scss` 第441-454行 `--semi-ai-general-7-0~3`）独立算了一遍白字对比度，结果同样是 1.57/1.62，与 lotus 实测几乎完全一致，证实这同样是 Semi 官方暗色模式本身的固有设计特征（不是 lotus 引入的额外劣化），同样不应被视为需要修复的缺陷。
