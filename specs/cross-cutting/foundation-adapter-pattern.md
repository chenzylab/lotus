...
- **拖拽排序实现中，若在 `mousemove`/`mouseup` 事件处理函数里才调用 `getBoundingClientRect()` 测量元素矩形，测到的是"当前已经应用了 CSS transform 之后的视觉位置"，不是"未拖拽时的原始布局位置"——用这份被污染的矩形去做目标索引判定，会导致被拖拽项在算法看来"已经漂移到了目标附近"，从而永远判定 `targetIndex === activeIndex`（因为拖拽项自己的矩形被自己的 transform 位移污染了，与自己的原始位置比较自然算不出偏移），松手后顺序完全不变**（踩坑 #88，TagInput 组件 draggable 拖拽排序功能排查发现）：现象是 `handleDragStart`/`mouseup` 两个事件确认都被正确触发（用 `window.__debug` 探针验证过完整的 dragState 流转），`computeTargetIndexWrap` 纯函数本身也已经过单测验证正确（20/20 通过），但真机测试里无论怎么拖拽，`targetIndex` 恒等于 `activeIndex`，标签顺序纹丝不动——排查时容易先怀疑"几何算法本身有 bug"（尤其是刚移植的新算法，第一反应是怀疑移植过程出错），但打印测量到的 `rects` 数组后发现被拖拽项的坐标是一个"介于起点和当前鼠标位置之间的漂移值"而非它原本该在的位置，才定位到根因：CSS transform 只改变视觉渲染位置，不改变元素在文档流中的布局占位，但 `getBoundingClientRect()` 返回的是渲染后的视觉矩形，包含 transform 造成的偏移。**修复方式**：拖拽开始（`mousedown`）那一刻测量一次全部标签的静态矩形并缓存到一个非响应式的模块级/闭包变量里（不是 `track()` 响应式变量，因为这份快照在整个拖拽周期内必须保持不变，不能随重渲染刷新），后续 `mousemove` 的目标索引预览和 `mouseup` 的最终判定都只读这份缓存，不再重新调用 `getBoundingClientRect()`。**通用教训**：任何"拖拽时改变了元素视觉位置（无论是 CSS transform 还是其它视觉位移手段）、又要用几何测量来判定目标位置"的实现，测量时机必须固定在"变形开始之前"，一旦开始施加位移就不能再信任实时 DOM 测量结果——这与 React/Vue 生态里 `dnd-kit` 等成熟拖拽库统一采用"`dragStart` 时快照初始矩形，全程复用"的设计是同一个道理，不是 lotus 自己独有的坑，但用纯手写拖拽实现（不借助第三方库）时容易在第一版实现里漏掉这一步，因为"每次测量都用最新的 `getBoundingClientRect()`"是更符合直觉但错误的写法。
- **`maxTagCount`/类似的"数量折叠 + 点击展开"UI 模式，若展开状态只是切换了一个"是否显示折叠气泡"的布尔量，却忘了同步切换"渲染列表用的是折叠后的子集还是完整数组"这一层，会导致点击"+N"后气泡消失了但列表长度完全不变**（踩坑 #89，TagInput 组件 maxTagCount 折叠展开交互排查发现）：现象是点击"+N"按钮后，`restExpanded` 状态确实切换了（`showFolded` 计算结果从 `true` 变为 `false`，"+N" 气泡正确消失），但可见标签数量仍然停留在折叠前的数量，展开操作视觉上"什么也没发生"（除了气泡消失）。根因是 `@for` 循环遍历的数据源从一开始就写死成了 `foldResult.visible`（`foldTags()` 纯函数算出的、恒定被截断到 `maxTagCount` 条的子集），而不是根据 `showFolded` 这个布尔量在"折叠子集"和"完整数组"之间切换——`showFolded` 只控制了"+N" 气泡本身的显示与否，从未真正驱动主列表的数据源切换，两处渲染逻辑各自为政。**修复方式**：新增一个显式的派生值（如 `visibleTags`），用三元表达式在 `showFolded ? foldResult.visible : state.tagsArray` 之间选择，`@for` 改用这个派生值而不是直接用 `foldResult.visible`。**通用教训**：任何"根据某个布尔态在两种数据源之间切换渲染内容"的场景，写代码时要问一句"这个布尔量真的驱动了数据源切换，还是只驱动了旁边一个装饰性 UI 元素的显隐"——本例中这两件事看起来是一体的（"折叠"应该同时意味着"数量少 + 有气泡提示"），但实现时很容易只处理了其中一半，尤其是当折叠后的子集数据（`foldResult.visible`）是从另一个纯函数直接算出、看起来"理所当然就是要渲染的数据"时，容易忘记它只在折叠态成立，展开态需要显式换成完整数据源。
- **组件的 `autoFocus` 类 prop 在挂载时调用 `element.focus()`，如果该元素当前不在浏览器视口内，浏览器会自动执行一次隐式 `scrollIntoView`（滚动整个页面/最近的可滚动祖先，把这个元素滚动到可见区域）——这不是 lotus 代码的 bug，是浏览器对"聚焦不可见元素"的原生规范行为，但在 playground 这类"同一个长页面里累积挂载几十个组件 demo"的场景下，会产生真实的、影响其它组件 e2e 测试的副作用：只要某个 demo 组件默认 `autoFocus=true`（忠实对齐 Semi 的默认值）、且这个 demo 在页面里的位置足够靠后（不在首屏视口内），挂载时就会把整个页面自动滚动到该组件所在的深度**（踩坑 #90，PinCode 组件排查发现，回归防护 BackTop 组件的 e2e 测试）：现象是新增 PinCode 的 5 个 demo 区块（其中默认参数保留了 Semi 的 `autoFocus=true` 语义）后，与 PinCode 完全无关的 `BackTop` 组件 e2e 测试开始稳定失败（5/5 复现）——`page.goto('/')` 后立即断言"未滚动时 `.lotus-back-top` 不存在"，实际读到 `window.scrollY` 已经是 33678.5（页面被自动滚动到了两万多像素深的位置），因为 `visibilityHeight`（默认 400px）阈值早已被这个隐式滚动越过，`BackTop` 组件判定"已经滚动过页面"从而提前显示。排查时容易先怀疑"BackTop 组件本身的滚动判定逻辑有回归"（尤其是它自己的测试改都没改却突然失败，第一反应是怀疑最近改过的公共基础设施），但实际上 BackTop 的代码完全没变，问题源头在页面级别的隐式滚动，属于"看起来毫不相关的两个组件之间通过共享页面产生的耦合"。**修复方式**：playground 里同一个组件的多个 demo 区块，只保留第一个（或明确要展示 autoFocus 效果的那一个）用默认 `autoFocus=true`，其余全部显式传 `autoFocus={false}`——甚至第一个也应该考虑是否要在长页面场景下关闭，因为它同样会把页面滚动到自己的位置，干扰后续依赖"页面初始滚动位置为 0"这一前提的其它测试。**通用教训**：任何会在挂载时调用 `.focus()`（不管是显式 autoFocus prop 还是别的自动聚焦时机）的组件，写 playground demo 时都要意识到这个副作用不只影响组件自身的展示，还会通过"隐式滚动整个页面"externality 影响同一页面上所有依赖"初始滚动位置"这类全局视口状态的其它组件测试——排查跨组件的、看似无关的 e2e 回归时，如果失败的测试断言的是全局状态（滚动位置、焦点归属、URL hash 等）而不是组件自身内部状态，要优先怀疑"是不是页面上新增的别的组件产生了副作用"，而不是先怀疑失败测试对应组件自己的代码出了回归。
- **把一个自定义子组件（如 `FilterMenu`）整体作为值传给另一个组件的 `content`/类似 prop（如 `<Popover content={<FilterMenu activeValues={draftFilterValues} .../>} />`），子组件内部读取的响应式变量（`draftFilterValues`）不会随后续变化重新驱动子组件重渲染，子组件看起来"卡在了初次挂载时的快照"**（踩坑 #91，重大，Table 组件筛选下拉菜单排查发现）：现象是点击筛选菜单里的 checkbox，`toggleFilterDraft` 函数确认被正确调用、`draftFilterValues` 这个 `track()` 响应式变量的值也确认发生了变化，但 checkbox 的 `checked` 视觉状态、以及最终点击"确定"后过滤结果都毫无变化，e2e 测试显示两次读取的 DOM `innerHTML` 完全一致。排查时先怀疑过 Popover 的 `trigger="custom"` 模式有 document 级点击外部自动关闭的副作用把面板关掉了（实际没有，`custom` 模式完全不含这类逻辑）、也怀疑过是不是 `content` prop 本身语法有问题——但同一代码库里 Select 组件同样把含 `@for` 循环的 `<ul>` 列表内联写在 `content={ <ul>...@for(){...}...</ul> }` 里且能正常响应式更新，证明"JSX 表达式作为 prop 传递"本身没问题。真正的差异是 Select 的例子是**内联**在调用处直接写 JSX（`@for` 本身处于外层组件的响应式渲染树里），而 Table 这里是把整棵子树包进了**另一个独立的自定义组件函数**（`FilterMenu`）、再把这个组件的**调用结果（element）**当值传给 `content`——`<FilterMenu activeValues={draftFilterValues} .../>` 这行代码本身是在 Table 组件的 `@{...}` setup 阶段只求值一次的普通表达式，构造出的 element 对象里 `activeValues` 是那一刻的值快照，不会因为 `draftFilterValues` 后续重新赋值而让这个已经构造好的 element 重新求值。**修复方式**：不把这类"内容会随外部响应式状态频繁变化"的浮层内容包装成独立子组件再通过 prop 间接传递，而是直接内联写在调用处的 `content={...}` 内部（参照 Select 组件的既有写法）——这样内部读取的响应式变量处于同一个响应式追踪范围内，能正确重新渲染。**通用教训**：`content`/`render` 这类"接收任意 JSX 作为浮层内容"的 prop，传入值如果是**跨组件边界构造好的 element**（尤其是从另一个自定义函数组件的调用结果），要假设它是一次性快照，不要指望它会随内部引用的响应式变量自动更新；只有直接内联（不经过额外的自定义组件函数调用）写在这个 prop 的表达式里，才能保证响应式链路不被切断。这与踩坑 #74（`track(() => foundation.method())` 因内部 `untrack()` 阻断依赖收集）是同一类"响应式链路被中间层悄悄切断"问题的另一种表现形式，但触发机制不同（#74 是 `untrack()` 主动阻断，#91 是跨自定义组件传递 element 值本身天然不具备响应式）。
- **父元素和它内部的子组件都各自绑定了功能重复的 `onClick`（如 `<li onClick={() => toggle(v)}><Checkbox onChange={() => toggle(v)}>...</Checkbox></li>`），点击时浏览器事件冒泡会让两个处理函数都被触发，对于"切换布尔值/数组成员"这类幂等性依赖单次调用的操作，两次调用会相互抵消，最终表现为"点击完全没有反应"**（踩坑 #92，Table 组件筛选下拉菜单排查发现，与踩坑 #91 在同一处代码里先后暴露）：现象与踩坑 #91 高度相似（checkbox 视觉状态不变、过滤不生效），排查时先定位到 #91 那层响应式阻断问题并修复（把 `FilterMenu` 内联展开），修复后症状依然存在，才发现这是两个独立叠加的 bug——用 e2e 测试直接对 checkbox 元素调用 `.click()` 并打印前后 `checked` 状态，发现从 `false` 变化到 `true` 又立即变回 `false`（在同一次点击事件里），而不是停留在初始值不变；进一步打印发现 `toggleFilterDraft` 被调用了两次：一次来自外层 `<li onClick>`，一次来自内层 `<Checkbox onChange>` 内部把原生 `click`/`change` 事件转换后再触发的 `onChange` 回调，两次调用里第一次把值从"未选中"翻转成"已选中"，第二次紧接着又翻转回"未选中"，净效果等于什么都没发生。**修复方式**：`<li>` 包裹一个自带完整交互能力（自身即可响应点击/触发变更回调）的子组件（如 `Checkbox`）时，不要在 `<li>` 自己身上再重复绑定同语义的 `onClick`——只保留子组件自身的事件回调（`onChange`），外层容器元素只负责布局/样式，不承担交互逻辑；如果确实需要"点击整行都能触发"的效果（比如让文字区域也可点击），应该让唯一的事件源头往下调用同一个处理函数，而不是在两个层级各自独立绑定。**通用教训**：任何"容器 + 内部可交互子元素"的组合，写事件绑定前先问一句"这个点击手势最终会经过几条事件路径到达处理函数"——原生 DOM 事件冒泡是默认行为，容器和子元素各自绑定同语义回调时，只要没有 `stopPropagation()`，点击子元素会同时触发两条路径；对于加法/追加类操作（如"打开弹层"）重复触发通常无害，但对于状态切换类操作（toggle/increment 后又 decrement）重复触发会产生"看起来完全没反应"这种极难第一时间联想到"事件触发了两次"的诡异症状，排查时如果确认处理函数被调用了但状态又"看起来没变"，要优先在处理函数入口打印调用次数/参数，而不是继续怀疑状态存储或渲染层出了问题。
- **首次引入运行时级第三方 JS 库依赖（CodeHighlight 组件，底层用 prismjs）时确立的架构先例**：lotus 此前所有组件（含 Cropper 这类"听起来会包一个库"的组件）都是自实现算法、零第三方运行时依赖，CodeHighlight 是第一个例外。做法是——Foundation 层（`resolveCodeClassName`）保持零依赖纯函数，不 import prismjs，只负责 `<code>` 元素的 class 名解析（`language-*`/`line-numbers`），与具体高亮库解耦；真正调用 `Prism.highlightElement()` 的逻辑完全下沉到 `.tsrx` 渲染层的 `effect()` 回调里，`ref` 拿到 DOM 节点后手动设置 `className` + `textContent`（纯文本写入，不经 `{@html}`/`innerHTML`，规避 XSS），再交给 Prism 原地高亮。语言包采用顶层同步 `import 'prismjs/components/prism-xxx.js'`（而非 chenzy.design Svelte 版本里的运行时懒加载 `import()`），因为 tsrx 组件函数体内不能 `await`、且懒加载引入的异步竞态在当前阶段收益不足以抵消复杂度，换成同步引入让实现和测试都更简单可靠，代价是最终包体积略增。**通用教训**：需要包装重度第三方库的新组件，优先复用这套分层——纯函数 Foundation 保持库无关（便于未来换库/测试），DOM 操作和第三方库调用留给渲染层的 `effect()`，语言包类"按需加载"需求在 tsrx 里默认从同步导入开始，除非包体积预算实测超标才升级到动态 `import()` 并重新验证 tsrx 异步边界规则。
- **tsrx 原生支持"标签名读运行时字符串/成员表达式"的动态标签语法 `<{expr}>...</{expr}>`（重要能力确认，MarkdownRender 组件排查发现）**：开发 MarkdownRender 前一度以为 tsrx 只能像普通 JSX 一样写死字面量标签名，担心递归渲染 hast 树（`{type:'element', tagName:'h1'|'p'|'table'|...任意字符串}`）时不得不写一条几十个分支的 `@if/@else if` 白名单分派链，或者退化成命令式 `document.createElement` 手动建树——后来验证确认 tsrx 完全等价于 Svelte 的 `<svelte:element this={tagName}>`：闭合标签必须重复同一个表达式（`<{node.tagName}>...</{node.tagName}>`），表达式可以是标识符或成员访问（`node.tagName` 合法，但函数调用/展开/字符串拼接不合法），运行时降级为 `composite()` helper，每次响应式依赖变化都会 `destroy_block` 清理旧 DOM 再重新 `document.createElement` 建新元素，清理/重建逻辑运行时内置不需要手写。lotus 的 `Tabs` 组件（`packages/ripple/src/navigation/tabs/index.tsrx`）早就在用 `<{item.render} />` 渲染运行时决定的组件值，是同一机制的既有先例，只是此前没人把它和"渲染任意标签名的 hast/AST 树"这个用例联系起来。**通用教训**：任何"要渲染的标签名/组件本身是运行时数据而非编译期常量"的场景（解析第三方 AST、渲染用户自定义 schema、动态表单等），第一反应应该是找 `<{expr}>` 动态标签语法而不是白名单分派或命令式 DOM 接管——后两者都是本可以避免的额外工作量。
- **递归渲染 hast/AST 树时，同一种节点类型（如 `<code>`）在树里可能以两种不同的父子关系出现——必须对每种出现形态都单独判断，不能只处理最常见的那种**（踩坑，MarkdownRender 组件排查发现）：markdown 里"围栏代码块"（```` ```ts ```` ）编译出的 hast 结构是 `<pre><code class="language-ts">`（`code` 节点包在 `pre` 里），而"行内代码"（`` `const a = 1` ``）编译出的 hast 结构是**独立的 `<code>` 节点，没有 `pre` 包裹**、直接作为 `<p>` 的子节点出现。递归渲染逻辑最初只在 `node.tagName === 'pre' && 含 code 子节点` 这一个入口里特殊处理代码块（转发给 `CodeHighlight`/`simple-code` span），行内 `<code>` 没有匹配到任何特殊分支，直接走了兜底的"未知标签动态渲染"逻辑，被渲染成裸的原生 `<code>` 标签（没有语言高亮也没有 `simple-code` 样式）——真机截图里肉眼可见"行内代码文字没有灰底样式"，但因为文字内容本身是对的，容易被误认为"只是样式没写全"而不是"根本没走对渲染分支"，e2e 测试用精确的 class 选择器断言才准确暴露了元素完全不存在。**修复方式**：在 `pre>code` 特判之外，额外增加一条独立的 `node.tagName === 'code'` 判断分支，两条分支共用同一个代码块渲染子组件（该子组件内部本来就用"有没有 `language-*` 类"来决定走高亮还是走 simple-code，天然覆盖了"行内代码没有 language 类"的情况，不需要额外分叉）。**通用教训**：把第三方生态（unified/remark/rehype 这类 AST 编译器）产出的树结构当"渲染规则表"来写分派逻辑时，不能只对着一个手写的示例输入验证——同一逻辑标签在不同上下文（块级 vs 行内、有无某个包裹层）可能产出的树形状不同，写分派条件前应该枚举该标签在生态惯例里的所有已知出现形态（本例只需搜一下"markdown 行内代码 vs 围栏代码块的 AST 结构差异"就能提前发现两种形态），而不是仅覆盖手写测试用例里凑巧出现的那一种。
- **异步编译型组件（渲染内容依赖 effect 里的 Promise 结果）的 e2e 测试，任何"立即读取一次快照再用普通断言比对"的写法（如 `await locator.allTextContents()` 后 `expect(arr).toEqual([...])`）在本地/低负载环境下能稳定通过，一旦扔进 CI 这类更慢、并发更高的环境就会把"编译还没跑完"的中间态当成最终结果误判失败——而同一个测试文件里紧邻的其它用例因为用的是 Playwright 自带 auto-retry 的断言（`expect(locator).toHaveText/toBeVisible/toHaveCount` 等，失败会在超时窗口内反复重试直到通过或超时）而完全不受影响，导致"同一文件大部分用例稳定绿、只有个别用例在 CI 偶发红"这种局部性很强的假象**（踩坑，MarkdownRender 组件 CI 排查发现，本地 `--repeat-each=10` 全绿但 CI 首次运行即失败）：`MarkdownRender` 的编译是 `effect()` 里对 `compileToHast()`（unified/remark 生态，本身还要动态 `import()` 加载编译器模块）发起的异步调用，写完 `raw` 到组件真正把 hast 编译结果渲染进 DOM 之间存在一段不确定时长的间隙，本地开发机通常够快、多次手测/重复测试很难撞上这个窗口，但 CI runner 的 CPU/IO 更慢、且多个测试并行抢占资源时这段间隙会被放大到肉眼可见。诊断关键：`gh run view <id> --log-failed` 里的失败断言是 `expect(received).toEqual(expected)`（jest 风格 deep equality，不是 Playwright 的 `toHaveText` 系列），且失败信息显示"收到空数组、期望非空数组"——这个形状(`toEqual` + 空结果) 本身就是"读取时机早于异步渲染完成"的强烈信号，应该优先怀疑时序竞态而不是逻辑错误。**修复方式**：把 `allTextContents()` 一次性读取改成先用 `expect(locator).toHaveCount(n)` 等到数量达到预期（这一步会自动重试直到编译完成或真正超时），再用 `expect(locator.nth(i)).toHaveText(...)` 逐项断言内容，全程留在 Playwright 的 auto-retry 断言体系内，不掺杂任何"立即读取值 + 普通深比较"的环节。**通用教训**：任何组件的渲染内容来自 `effect()`/`$effect` 里发起的异步操作（`fetch`、动态 `import()`、`Promise` 链）时，对应的 e2e 断言必须从头到尾只用 Playwright 自带的 auto-retry locator 断言，一旦某一步先用 `await locator.xxx()` 拿到一个 JS 原始值（数组/字符串/数字）再交给 `expect(value).toEqual(...)` 做静态比较，这一步就丧失了重试能力，会在异步链路较慢的环境（尤其是 CI）里变成不稳定的失败点——本地测试环境的速度差异不能作为"这个写法是安全的"的证据，必须假设生产 CI 环境比本地慢得多来设计断言写法。
- **`@if(){...}`/`@else{}` 语句块内，只要某个分支要输出多于一个的顶层兄弟节点（不含它自己的嵌套子节点），这个分支本身也必须用 `<>...</>` fragment 包一层，否则 tsrx 编译器报"A code block renders a single node"——这条规则此前只在组件函数体本身的顶层验证过（`@{}` 语句容器顶层要包 fragment，Nav 组件踩坑 #16/#17 记录过），本次发现它对每一层嵌套的 `@if`/`@else` 分支都同样成立，不是只在最外层生效一次**（踩坑，JsonViewer 组件排查发现）：`JsonNodeView` 组件最初的写法是 `@if (isContainer) { <span>...</span> @if(...){两个span} <span>...</span> ... }`——最外层 `@if (isContainer)` 分支内部一共有五段左右并列的输出（switcher span、可选的 key+colon、bracket span、可选的折叠摘要、可选的展开子树），只给内层几个 `@if` 分支包了 `<>`，唯独漏了给最外层 `isContainer` 分支本身包一层，typecheck 反复报同一句报错且不带行号/列号，一度怀疑是内层某处遗漏，用最小复现（`<li>@if(x){two spans}</li>`）二分确认"任意一层多节点 `@if` 分支都需要包 fragment"这条规则本身，而不是找漏改的那一处。**修复方式**：给最外层 `@if (isContainer) {...} @else {...}` 的两个分支各自整体包一层 `<>...</>`（哪怕分支内部已经有自己的子级 `@if` 各自包好了 fragment，外层分支自己仍然需要）。**通用教训**：写多层嵌套 `@if`/`@else` 且任意一层可能输出多个兄弟节点时，不要只顾着给"看起来有问题"的那一层加 fragment，应该系统性检查从最内层到最外层的每一层——每一层只要它自己（不算子层）的直接输出多于一个节点，就必须包 `<>`，与它的父层/子层是否已经包了无关，是各自独立的编译期要求。
- **组件函数体（`@{...}`）顶层的 `const x = expr` 只在组件首次挂载时求值一次，如果 `expr` 依赖的是会随后续 props 变化而变化的值（尤其是从父组件"整体替换引用"传下来的响应式集合/对象，如 `Set`/`Map`/普通对象），这个 `const` 之后永远不会重新计算，必须用 `track(() => expr)` 包裹才能在依赖变化时重新求值**（踩坑，JsonViewer 组件排查发现，真实功能性 bug，非编译期报错）：`JsonNodeView` 组件里 `const expanded = expandedPaths.has(node.path)` 写在函数体顶层，`expandedPaths` 是父组件 `JsonViewer` 传下来的 `state.expandedPaths`（每次展开/折叠操作都会整体替换成一个新的 `Set` 引用）。真机测试点击展开箭头后 DOM 完全没有任何变化（`innerHTML` 长度前后一致），但 `onClick` 回调本身确认被正确调用、Foundation 的 `handleToggleExpand` 确认正确更新了父组件的 `state`——问题在于子组件 `JsonNodeView` 收到新的 `expandedPaths` prop 后，那句顶层 `const expanded = ...` 根本没有重新执行，子组件读到的永远是挂载那一刻算出的旧布尔值。这与"响应式阻断"系列踩坑（#74/#78/#85，`track()` 内部经 `untrack()` 取值导致依赖收集被阻断）表面相似（都是"数据变了但 UI 没变"）但**根因完全不同**——那几次是响应式依赖收集被主动阻断，这次是压根没有建立响应式依赖（从来没有被 `track()` 包裹过，是一次性的普通变量赋值）。**修复方式**：把 `const expanded = expandedPaths.has(node.path)` 改成 `let &[expanded] = track<boolean>(() => expandedPaths.has(node.path))`。**通用教训**：组件函数体顶层的 `const`/`let`（非 `track()`）赋值本质上是"挂载时快照"，只要右侧表达式引用了任何 prop（哪怕只是调用它的方法如 `.has()`/`.get()`，不是整体解构赋值），就要默认它不会随 props 更新重新求值，除非确认这个值本身是"仅依赖挂载时不变的静态数据"（如本例里同样写在顶层但安全的 `isContainer`/`bracketOpen`/`bracketClose`——它们只依赖 `node.type`，而每个 `JsonNodeView` 实例对应的 `node` 引用在其生命周期内不会变，这类值留作普通 `const` 没问题）。判断标准是"这个值依赖的 prop 在组件存活期间会不会被父组件换成新引用/新值"，会的话就必须 `track()`，不会的话留普通变量即可，不能凭"看起来是个简单表达式"就默认安全。
- **`track(() => foundation.getXxx())` 因方法内部经 `this.getState()`（被 `untrack()` 包裹）取值而阻断响应式依赖收集——这是踩坑 #74/#78/#85/JsonViewer 那次的第 N 次复现，本次在 AudioPlayer 组件的曲目切换场景暴露，说明这个坑不是"记住教训就不会再犯"，而是每次写新的 `track(() => foundation.xxx())` 调用点都要重新自查**（踩坑，AudioPlayer 组件排查发现）：`let &[currentTrack] = track(() => foundation.getCurrentTrack())`，`getCurrentTrack()` 内部是 `this.tracks[this.getState().currentTrackIndex]`——`this.getState` 在 Adapter 里实现为 `() => untrack(() => state)`，导致这个 `track()` 从未真正收集到 `currentTrackIndex` 变化的依赖。真机测试点击"下一曲"按钮后，Foundation 的 `changeTrack` 确认正确把 `state.currentTrackIndex` 从 0 改成了 1、audio 元素的 `src` 也确认被效果里的另一段代码正确同步切换了，但曲目标题 UI（`currentTrack?.title`）死活不变，两次测试文本完全相同。**修复方式**：不直接把 `foundation.getCurrentTrack()` 包进 `track()`，而是把归一化后的曲目数组本身也做成一个响应式 `track()` 变量（`let &[tracksList] = track(() => foundation.getTracks())`，在 `audioUrl` 变化的 `effect()` 里显式重新赋值），再用 `track(() => tracksList[state.currentTrackIndex])` 直接读 `state.currentTrackIndex`（响应式变量本身，不经过 Foundation 方法调用）计算出 `currentTrack`。**通用教训**：每写一个新的 `track(() => foundation.methodName())` 调用点，要养成机械式自查的习惯——点开这个方法的实现，看它内部是不是经 `this.getState()` 取值；只要是，就必须重构成"track() 计算体直接读 state 的字段本身，结果只喂给不经过 Foundation 实例方法调用的模块级纯函数或数组索引"，不能因为"这次调用看起来很简单/是只读查询"就跳过检查——这条坑已经在 Tree、Table、JsonViewer、AudioPlayer 至少四个组件里独立复现过，本质原因是"包一层方法调用"这个写法本身在视觉上和"直接读响应式变量"几乎没有区别，容易被无意识地写出来。
- **测试环境里"伪造 `loadedmetadata` 事件但 `<audio>` 元素并无真实可寻址的媒体流"这种半伪造状态下，`el.currentTime = X` 赋值会被浏览器静默钳制到一个远小于 X 的内部值（不是抛错、不是保持 0，是钳到一个看似随机实则由浏览器媒体子系统内部状态决定的具体数字），且赋值动作仍然会正常派发一次 `timeupdate` 事件——如果组件的 `timeupdate` 处理器信任这个事件里的 `el.currentTime` 作为权威值去覆盖此前设置的乐观状态，就会在测试环境里复现出"读起来毫无规律、但每次代码路径完全相同"的确定性错误结果**（踩坑，AudioPlayer 组件 e2e 排查耗时最长的一次，历经十余轮系统性二分排查）：现象是点击进度条中点，Foundation 内部日志显示 `ratioFromEvent` 计算出的比例、`seekTo` 钳制后的返回值全部逐次核对无误（稳定输出 50），但最终 DOM 显示的时间文本在不同测试运行里在 `0:50` 和 `0:02` 之间不确定地摇摆，一度怀疑是 Playwright 虚拟鼠标位置跨 `page.goto()` 保留导致的坐标污染、`scrollIntoViewIfNeeded()` 滚动未完成的布局竞态、tsrx 响应式阻断等多种假设，逐一用最小复现脚本证伪后，才用 `el.evaluate(el => { el.currentTime = 50; return el.currentTime })` 直接在浏览器侧验证出赋值前后不一致（读回 2 而非 50，`readyState` 只有 1/HAVE_METADATA），坐实了根因在于"标准 HTML5 audio 语义下 `currentTime` 赋值需要真实可寻址的媒体数据支持，纯靠 `Object.defineProperty` 伪造 `duration` 骗过 UI 层，并不能让 `currentTime` 的原生 setter 一并被绕过"——这与真实浏览器环境下用户拖拽进度条完全正常（`currentTime` 赋值真正生效，`timeupdate` 回读的值与设置值一致，感知不到任何覆盖）形成对照，是测试环境保真度不足暴露出的问题，不是组件的"乐观更新 + 媒体元素校正"架构设计缺陷。**修复方式**：`simulateLoadedMetadata` 测试辅助函数里，除了给 `duration`/`volume`/`playbackRate` 用 `Object.defineProperty` 伪造只读属性外，把 `currentTime` 也整个替换成一个自建的 getter/setter 对（内部用普通闭包变量存值，setter 里手动派发 `timeupdate` 事件），彻底绕开浏览器原生 audio 元素对 `currentTime` 赋值的媒体子系统前置校验。**通用教训**：给原生媒体元素（`<audio>`/`<video>`）的属性做测试桩时，"这个属性看起来是简单的可读写字段"不代表它真的是——`currentTime`/`play()`/`pause()` 这类方法在规范里都隐式依赖 `readyState`/真实解码进度，只伪造上层可见的属性（`duration`）无法让底层依赖链条的属性（`currentTime`）跟着可用，必须识别出这类"表面独立、实则耦合"的属性组，要么把整条链路都伪造透彻，要么在实现代码里对这类边界情况做防御性判断，两者选一，不能只做一半。
- **`{...expr}` spread 属性展开语法，当 `expr` 求值为 `undefined` 时会让整个组件收到的 props 对象本身变成 `undefined`（不是"忽略这次 spread、其余具名 prop 正常传入"），组件解构 props 时对 `undefined` 做默认值判断（tsrx 编译产物内部用 `in` 操作符检查字段是否存在）会直接抛出未捕获的 `TypeError`，且这个异常会中断当前渲染分支所在的整个父级 block，导致同一渲染树里其它完全无关的兄弟组件（包括不同子树的组件）也一并渲染失败**（踩坑 #93，重大，Sidebar 组件排查发现，耗费大量排查时间因为表现酷似框架级限制）：`<JsonViewer value={item.content} {...item.jsonViewerProps} />`，`item.jsonViewerProps` 在未显式设置时是 `undefined`，运行时报错 `Cannot use 'in' operator to search for 'showCopy' in undefined`（发生在 `JsonViewer` 组件 `showCopy = true` 默认值解构处）。排查过程极具迷惑性：真正受害的是页面上另一个完全独立的自定义组件（`SidebarHeader`，通过 `header={<SidebarHeader .../>}` 作为另一个组件 `Container` 的 attribute 值传递），它连同其内部的返回按钮、关闭按钮全部不渲染，但组件函数体本身确认被调用（`console.log` 能打印、Foundation 状态确认正确）——由于失败现象高度符合"自定义组件作为 prop 值传递、其内部 `@if` 语句渲染失效"这一 tsrx 框架级限制的特征（且排查中一度深入阅读了 `@tsrx/ripple`/`@tsrx/core` 编译器源码试图从 `has_template`/`is_template_value_position` 等编译期分析逻辑里找根因），几乎得出"这是框架限制、需要架构性绕过（放弃 header 自定义组件方案）"的错误结论；直到改用独立的 Playwright 测试脚本（`page.on('pageerror', ...)` 监听未捕获异常）而非仅凭 DOM 查询和已重复读取产生噪声的浏览器扩展控制台，才捕获到真正的异常堆栈——`JsonViewer` 崩溃发生在与 `SidebarHeader` 完全不同的子树（`mode==='code'` 分支的 `CodeContent` 内部），两者仅仅共享同一个父级渲染 block，一个的未捕获异常导致整个 block 的渲染工作被打断，殃及池鱼。**修复方式**：`{...(item.jsonViewerProps ?? {})}`，任何 spread 到组件 prop 上的表达式，只要其来源（对象字段、可选参数）不能 100% 确定非空，一律显式 `?? {}` 兜底，不能假设"没传就相当于没有这个 spread"。**通用教训**：(1) tsrx（以及大概率其他基于类似编译期 JSX-to-DOM 转换的框架）对 spread 到 `undefined` 上的容错程度可能远低于 React（React 里 `{...undefined}` 是安全的空操作），必须显式空对象兜底，这是每次写 `{...possiblyOptional}` 都要做的机械检查；(2) 一个组件抛出的未捕获异常在这类框架里可能不会被限制在该组件自己的渲染范围内，而是会向上炸穿整个 block/scope，破坏兄弟组件的渲染——诊断"看起来不相关的多个组件同时渲染失败"时，第一步永远应该是找一个可靠的异常捕获通道（`page.on('pageerror')`、`window.onerror`，而不是反复用 DOM 查询猜测"渲染了没有"）确认是否存在被吞掉的运行时异常，再决定要不要怀疑框架限制或架构设计——本次排查如果一开始就监听 `pageerror` 而不是仅靠 `document.querySelector` 反复试探，能把排查时间从近一小时缩短到几分钟内定位根因；(3) 浏览器自动化工具（本例中 claude-in-chrome 扩展）连接不稳定、reused console 历史消息残留等环境噪声会进一步误导排查方向（曾经把旧 tab 里的历史报错误判为当前代码的新报错），关键排查步骤应该切换到更可靠的自动化通道（本例用项目已有的 Playwright + `webServer` 配置写一次性调试脚本）来获得干净、可复现的信号。
- **`Button` 组件内部把 `disabled`/`loading` 这两个只读 prop 用非函数形式 `track<ButtonState>({ disabled, loading })` 包了一层内部 state，只在挂载时读一次快照；`ButtonFoundation.handleClick` 判断"是否要拦截这次点击"时读的是这个内部 state 而不是外部 prop 本身，一旦 `disabled` 在挂载后从 `true` 变为 `false`（如"发送"按钮随输入内容变化才可点击），原生 `<button disabled>` 属性和视觉 class 确认正确同步了，但 `onClick` 回调永远因为读到挂载时刻的旧 `disabled=true` 快照而被静默拦截、绝不执行——这是全部 `Button` 消费方共享的组件级 bug，不是任何单个消费组件自己的问题**（踩坑 #94，重大，AiChatInput 组件排查发现，殃及所有"disabled 会动态变化"的 Button 用法）：现象是 AiChatInput 的"发送"按钮在真机测试里 `disabled` 属性/class 确认已经动态变为可点击态（`getAttribute('disabled')` 返回 `null`），`elementFromPoint` 确认点击命中的就是按钮本身，原生 `click` 事件也确认冒泡到了 `document`，但 `onClick={handleSend}` 绑定的回调函数体内插的 `console.log` 死活不打印——反复怀疑过两层嵌套 `Popover` 的 trigger span 遮挡、`tsrx` 事件绑定失效、`Button` 组件本身 `onClick` 转发链路断裂，逐一用最小对照测试证伪（同组件树内 `Dropdown` 内部按钮的 `onClick` 确认能正常工作），最终读 `packages/ripple/src/basic/button/index.tsrx` 源码定位到 `let &[state] = track<ButtonState>({ disabled, loading })`——这是非函数字面量初始化，只在组件函数体首次执行时对 `{ disabled, loading }` 求值一次，之后 `disabled` prop 无论怎么变化都不会同步进这个内部 `state`，而 `ButtonFoundation.handleClick` 恰恰是 `const { disabled, loading } = this.getState(); if (disabled || loading) return;`，读的正是这个从未更新过的内部快照。**修复方式**：不再维护这份多余的内部 `state` 快照，`Adapter.getState` 直接实时返回 `{ disabled, loading }`（外部 prop 本身，天然响应式，读取时永远是当前值），`setState` 留空操作（`ButtonFoundation` 本身不会主动调用 `setState`，只读不写）——`getState: () => ({ disabled, loading }), setState: () => {}`。**通用教训**：(1) `let &[state] = track<T>({ ...expr })`（花括号对象字面量、非箭头函数）这种写法只是"用当前作用域这些变量的值构造一个对象快照"，与 `track<T>(() => ({ ...expr }))`（函数形式，每次依赖变化重新求值）是完全不同的两种语义，凡是想让内部 state 的初始值"继承外部 prop 但之后不再跟随 prop 变化"才应该用非函数形式，如果 Foundation 层的判断逻辑会读这份 state 来决定行为分支（如是否拦截点击），就必须确认这份 state 是否需要保持与 prop 同步；(2) 更通用的是，"只读、无内部可变状态的 UI 组件"如果只是把 prop 转发给 Foundation 做一次性判断（没有自己的状态转换要维护），根本不需要经过一层 `track()` 内部 state 包装，`Adapter.getState` 直接实时读 prop 闭包变量是更简单且不会过期的写法，只有当 Foundation 确实需要维护"不能仅由 prop 单向推导"的内部状态机（如 Tooltip 的 visible 受控/非受控双模式）时才值得引入内部 `track()` state；(3) 这类 bug 的症状（点击事件确认到达了正确元素、但绑定的回调死活不执行、没有任何报错）与"响应式依赖阻断"系列（#74/#78/#85/#92）表面相似（都是"看起来正确却毫无反应"），但根因层级更底层——不是渲染层没重新渲染，而是**事件处理函数内部的业务判断逻辑读到了过期数据直接短路返回**，排查时应该在可疑的回调函数体最开头就打印一行日志，快速确认"函数到底有没有被调用/在哪一行提前返回"，而不是一直停留在"检查 DOM 结构/CSS 遮挡/事件绑定是否正确"这个更表层的层面。
- **tiptap 自定义节点 `addAttributes()` 里，某个 attribute 的 key（如 `value`/`label`）如果不显式提供 `parseHTML`，tiptap 默认行为是用这个 key 本身去 `element.getAttribute(key)` 读取——如果对应的 `renderHTML`/外部构造的 HTML 字符串写的是 `data-value`/`data-label` 这类加了 `data-` 前缀的属性名，序列化输出没问题（`renderHTML` 里显式指定了 `data-value`），但反向解析（HTML 字符串插入编辑器时被 parse 回节点属性）会读取失败，默认值永远生效，实际内容丢失**（踩坑 #95，AiChatInput 组件排查发现）：`SkillSlot` 节点的 `addAttributes()` 里 `value: { default: '' }` 和 `label: { default: '' }` 都没写 `parseHTML`，只有 `hasTemplate` 一项显式写了 `parseHTML: (el) => el.getAttribute('data-template') === 'true'`——而配套的 Foundation 层纯函数 `getSkillSlotHTML(skill)` 生成的 HTML 字符串是 `<skill-slot data-value="..." data-label="..." data-template="...">`。通过 `editor.chain().insertContent(getSkillSlotHTML(skill)).run()` 插入这段 HTML 后，tiptap 用默认解析规则找 `el.getAttribute('value')`/`el.getAttribute('label')`（没有 `data-` 前缀），永远读到 `null`，两个 attribute 都回退到空字符串默认值——真机测试点击技能面板选中"翻译"后，编辑器里确实插入了一个 chip 节点（说明节点类型注册、`renderHTML`/NodeView 渲染管线本身都工作正常），但 chip 内部的 label 文本是空白，容易被误判成"NodeView 渲染逻辑有问题"而不是"attribute 解析这一步就已经丢了数据"。**修复方式**：给 `value`/`label` 都补上对应的 `parseHTML: (el) => el.getAttribute('data-value')` / `el.getAttribute('data-label')`，与已经正确的 `hasTemplate` 写法保持一致。**通用教训**：tiptap 节点如果采用"HTML 字符串手工拼接插入"（而非 `editor.commands.insertContent({type, attrs})` 这种直接传结构化 attrs 对象的方式）这条路径来构造节点实例，`renderHTML` 序列化用的属性名和 `addAttributes` 里每一个 attribute 默认解析规则用的属性名必须逐项核对一致——只要两者有一个不用默认 key 名（比如为了避免和标准 HTML 全局属性冲突而加了 `data-` 前缀），就必须给这个 attribute 显式写 `parseHTML`，不能因为"其中一项已经写对了"就假设其余同类属性也顺带写对了，需要逐项检查而不是抽查一项。
- **Modal/SideSheet 的焦点管理 `effect()` 如果依赖 `visible` 这个 prop 触发重新计算，会在 `@if(shouldRender)` 分支刚渲染出真实 DOM 的同一时刻读到 `ref` 回调还没来得及写入的 `null` 容器引用——必须让 effect 依赖响应式的容器节点变量本身（而非 `visible`），才能在 DOM 真正挂载完成后再执行焦点移入逻辑**（踩坑 #96，Modal/SideSheet 补齐无障碍焦点管理时排查发现）：最初写法是 `let contentNode: HTMLDivElement | null = null;`（普通变量，非 `track()`）+ `effect(() => { if (!visible) return; const container = untrack(() => contentNode); ... })`——这个 effect 的响应式依赖只有 `visible`，`visible` 从 `false` 变 `true` 的那一刻，`@if(shouldRender)` 分支才刚开始渲染 DOM 树，其中 `ref={(node) => { contentNode = node; }}` 回调要等到这次渲染真正跑到这一行才会执行，而 effect 的依赖收集触发时机早于此，`untrack(() => contentNode)` 读到的仍是挂载前的 `null`，导致"初始焦点移入""Tab 焦点陷阱"两个功能全部悄无声息地不生效（真机测试用 Playwright 验证 `initial focus inside modal` 稳定为 `false`，但代码逻辑本身完全正确、没有任何报错，是最容易被误判为"逻辑写错了"而不是"响应式时序问题"的一类 bug）。**修复方式**：把 `contentNode` 从普通 `let` 改成 `let &[contentNode] = track<HTMLDivElement | null>(null)`，effect 依赖体里直接读 `contentNode`（响应式变量，不经 `untrack`），这样 ref 回调执行时对 `contentNode` 的赋值会重新触发这个 effect（此时容器已经是真实存在的 DOM 节点），焦点移入才会在正确的时机发生；`handleContentTransitionEnd` 里的焦点归还逻辑因为绑定在 `onTransitionEnd` 事件回调而非 effect 里，不受这个时序问题影响。**通用教训**：任何"打开态才需要操作内部 DOM 节点"的 effect（聚焦、测量尺寸、绑定内部滚动监听等），如果这个内部节点是通过 `ref` 回调异步获得的，effect 的响应式依赖必须挂在"节点本身"上而不是"外部驱动开关的 prop"上——用 `visible`/`open` 这类 boolean prop 做依赖，本质上是假设"prop 变化的那一刻，所有派生的 DOM 结构已经就绪"，这个假设在 `@if` 条件渲染场景下不成立，只有当计算体本身读取了节点变量、且节点变量是响应式的，才能保证依赖收集和"节点真正可用"这两个时间点对齐。
- ~~Popconfirm 的 onVisibleChange/Esc 缺口~~：已修复。原缺陷：非受控模式下 `handleConfirm`/`handleCancel` 直接调用 `Foundation.setState({visible:false})` 绕开组件层的 `setVisible()`，导致 `onVisibleChange` 在确认/取消关闭路径上完全不触发；且 Popconfirm 固定用 `trigger="custom"` 导致继承的 Popover Esc 逻辑里 `!isControlled` 判断恒假，完全没有路径支持 Esc 关闭。**修复方式**：(1) `PopconfirmFoundation.handleConfirm`/`handleCancel` 改签名为接受一个 `onClose` 回调，成功关闭时调用 `onClose()` 而非自己 `setState`，`.tsrx` 渲染层传入 `() => setVisible(false)`，同时统一了受控/非受控两种模式的通知路径；(2) 在 `packages/ripple/src/feedback/popconfirm/index.tsrx` 组件层自己加一个 `window` 级 `keydown` 监听处理 Esc（对齐 Modal 已有的同类模式），不依赖 Popover 内部对 `trigger==="custom"` 的受控判断——那个判断服务于 Dropdown/Select 这类真正需要外部接管开关状态的场景，Popconfirm 只是借用 custom trigger 机制自己维护开关状态，语义不同，不能共用同一个判断分支。e2e 覆盖新增 2 用例（`e2e/feedback/popconfirm.spec.ts`），`--repeat-each=5` 35/35 全过。
- **拖拽实现里把元素的视口坐标（`getBoundingClientRect().left/top`）当成 CSS `style.left`/`style.top` 需要写入的坐标系使用——这两个坐标系只有在元素的 `offsetParent` 恰好是视口原点（无滚动、无定位祖先偏移）时才数值相等，一旦不满足就会产生一个恒定的系统性偏差**（踩坑，DragMove 组件排查发现，真实的坐标系混淆 bug）：`onDragStart` 计算 `startOffsetX = mousedown 时的 clientX - 元素左边缘` 这一步，最初错误地用 `getBoundingClientRect().left`（视口坐标）代入，而 `onDragMove` 算出的 `newLeft = clientX - startOffsetX` 结果又被直接写入 `element.style.left`——`style.left` 的坐标系是"相对 `offsetParent`"，不是"相对视口"，两者一旦不重合，算出来的新 `style.left` 就带着一个恒定的坐标系偏移量。现象极具迷惑性：e2e 测试连续两段等距拖拽（各移动 20px），第一段观测到位移 61px 而非预期 20px，但**第二段却精确是 20px**——因为第一段的绝对位置计算里混入了坐标系偏移常数，但第二段是"上一次算出的新位置"再叠加一次相对位移，偏移常数在两次计算的差值里被抵消掉了，只有"连续第一次挪动"这一步会看起来出错，容易被误判成"随机的测试环境抖动"而不是"确定性的架构问题"。定位方法：对照 Semi 源码 `_calcOffset` 逐行核对，发现 Semi 用的是 `this.element.offsetLeft`（DOM 原生属性，与 `style.left` 同一坐标系）而非 `getBoundingClientRect()`，才确认是自己移植时的坐标系选型错误，不是算法逻辑本身的问题。**修复方式**：`onDragStart` 的 absolute 定位策略分支，起始偏移量计算一律使用 `element.offsetLeft`/`element.offsetTop`（不是 `getBoundingClientRect()` 的视口坐标），只有 relative 策略下计算"约束容器与元素的真实几何差"这类跟 `style.left` 无关、纯粹用于范围判断的场景才继续用视口矩形。**通用教训**：任何要把"鼠标位置差值"换算成"要写入某个 CSS 定位属性的新值"的场景，第一反应要确认参与运算的所有坐标是否都在同一个坐标系里——`clientX/clientY`（视口）、`getBoundingClientRect()`（视口）、`offsetLeft/offsetTop`（相对 offsetParent）、`style.left/top`（相对 offsetParent，与 offsetLeft/Top 同系）是两组不同坐标系，混用产生的偏移量在简单场景（页面顶部、无滚动、offsetParent 就是 body）里可能恰好数值一致而被掩盖，必须在有滚动条或嵌套定位容器的真实场景下才会暴露，写单测时也要显式覆盖"两次连续位移"而非只测"单次位移是否正确"，因为常数偏移量在连续相对位移的差值计算里会被静默抵消。
- ~~Select 的 `selectedLabels[0]!.label` 响应式竞态~~：已修复。原缺陷：`packages/ripple/src/input/select/index.tsrx` `@if (selectedLabels.length > 0) { <span>{selectedLabels[0]!.label}</span> }` 里 `length > 0` 判断和 `[0]!` 非空断言分别对 `selectedLabels`（一个 `track` 派生值）独立求值——tsrx 编译后 `@if` 条件和分支体落在不同的响应式代码块里，`get_derived` 每次访问都会先检查依赖是否变化再决定是否重新计算，若两次访问之间 `optionList`/`displayValue` 恰好变化，会读到两次不同时刻的快照，并发跑多个 e2e 文件时命中过一次 `Cannot read properties of undefined (reading 'label')`。**修复方式**：新增一个 `singleSelectedLabel` 派生值只在单选场景下计算一次（`selectedLabels.length > 0 ? selectedLabels[0]!.label : undefined`），模板里 `@if (singleSelectedLabel !== undefined)` 判断和 `<span>{singleSelectedLabel}</span>` 渲染引用的是同一个变量，不再对 `selectedLabels` 做两次独立访问。**排查全代码库同类模式的方法论**：全局搜索"`@if(X.length>0)` 紧跟分支体内 `X[0]`"这个语法形状，逐一核实两个必要条件是否同时成立才判定为真实风险——(1) `X` 是否是响应式派生值（而非静态 props/常量数组）；(2) `X` 的值是否可能因外部实时数据竞态在两次访问间从非空变空（而非像 `UserGuide` 的 `currentStep = steps[displayCurrent]` 那样有严格边界保护、`steps` 非空时索引永远合法）。本次排查确认 Cascader/DatePicker/Upload/UserGuide/OverflowList/Table 里同形状的写法均不满足这两个条件（要么不做索引访问、要么有边界保护、要么是静态数据），只有 Select 这一处是真实风险，不需要不加区分地把所有 `length>0` 判断都改写。
- **Foundation 层的 `opts`（`min`/`max`/`step` 这类构造时固定的配置项）挂载后不响应 prop 变化是全代码库统一的既定架构，但方向（RTL/LTR）是运行时可能真实切换的全局状态（用户切换 UI 语言、`ConfigProvider.direction` 变化），如果照搬"固化不响应"的既定模式，会导致已经渲染在页面上的组件在方向切换后进入"数据和视觉不同步"的真实 bug，而不只是"设计上的简化"**（踩坑 #98，重大，Slider RTL 支持排查发现，耗费大量真机验证时间因为现象一度呈现为"随机不稳定"而非确定性错误）：Slider 补 RTL 支持时，最初在 `SliderFoundationOptions` 加了 `isRtl` 字段，用 `isRtl: untrack(() => locale).dir === 'rtl'` 在组件挂载时求值一次传入 Foundation 构造函数——这完全对齐了 `min`/`max` 等字段的既有写法，typecheck、单元测试、以及"静态检查手柄位置与数值是否数学自洽"这几类验证方式全部通过，一度被误判为"已经正确实现"。真正暴露问题的是用 ego-browser 做端到端真机验证时的操作顺序：playground 里 Slider 组件本身常驻不重新挂载，用户点击"切换到 rtl"按钮只是让 `ConfigProvider.direction` 响应式更新，此时 Slider 早已完成挂载、`foundationOpts.isRtl` 已经固化成了挂载那一刻的（`ltr`）值，后续无论怎么切换方向、怎么拖拽，Foundation 内部用的永远是过期的旧值——点击轨道物理左边 10% 位置得到的是纯 LTR 语义的数值 10，而不是 RTL 预期的 90。排查过程极具迷惑性：早期几轮用 CDP 合成鼠标事件模拟连续拖拽时，观测到的方向时对时错（有时候左拖增大符合预期，下一次左拖却减小），一度怀疑是"`dragMouse` 辅助函数状态残留"或"合成事件时序竞态"这类测试工具本身的问题，差点把一个确定性的代码 bug 归咎为环境噪声——直到改用"点击轨道任意点直接跳转"（不依赖拖拽的连续状态，每次都是独立、确定性的单次事件）而不是"拖拽手柄"这种依赖前序状态的交互方式做验证，才把现象从"看起来随机"收敛成"稳定复现 LTR 行为"，从而排除了测试工具的干扰，坐实是代码本身的方向没生效。**修复方式**：不重新构造 Foundation 实例，而是用一个 `effect(() => { foundationOpts.isRtl = locale.dir === 'rtl'; })` 直接修改 `foundationOpts` 对象的属性——因为 `SliderFoundation` 构造函数内部是 `this.opts = opts`（保存的是同一个对象引用而非深拷贝），修改这个共享引用的属性会立即反映到 Foundation 内部下一次方法调用读到的值，不需要引入"更新 Foundation opts"这类全新的机制。**通用教训**：(1) 引入任何 Foundation opts 字段之前，先问一句"这个配置在真实使用场景里，是只在组件生命周期开始时确定一次（如 `min`/`max`/`step`，产品设计上本来就不支持运行时切换），还是可能在组件已经挂载、且用户仍在与之交互的情况下发生变化（如方向、语言、主题这类全局环境状态）"——后者必须显式做响应式同步，机械照搬既有字段"构造时固化"的写法会引入真实 bug，而不是被"架构一致性"这个理由正当化；(2) 涉及方向感知的拖拽类组件（Slider 及未来可能出现的同类组件），验证 RTL 行为时优先用"点击跳转"这类无状态、单次确定性的交互路径先验证核心方向逻辑是否生效，再验证"拖拽"这类依赖 mousedown→mousemove→mouseup 完整状态机的连续交互——如果连续拖拽的观测结果不稳定，不要急于怀疑测试脚本或环境噪声，先用最简单的单次交互路径把"核心方向逻辑到底生不生效"这个问题隔离出来，再回头排查更复杂交互路径里的问题；(3) 静态状态下"数值与视觉位置是否数学自洽"这类验证只能证明"某一个固定时间点的快照是自洽的"，不能证明"运行时状态变化后系统仍然正确响应"——这是两种完全不同的验证目的，只做前者会让固化 bug 被彻底遗漏。
- **tsrx 的 scoped CSS 只对组件自身模板中直接渲染的节点生效，不会传导进被这个组件调用的子组件——把某个 CSS class 名对应的视觉规则留在"渲染这个 class 的元素实际所属的子组件"之外的地方（比如留在调用方/父组件的 `<style>` 块里），这条规则会在真机渲染时完全不生效（既不是被覆盖，是选择器压根匹配不上），且 `tsrx-tsc` typecheck 和 `pnpm lint` 都测不出来**（踩坑 #97，重大，TreeSelect 虚拟化改造排查真实点击定位偏移 bug 时发现，同一缺陷同时影响 Select/Cascader）：Select/Cascader/TreeSelect 三个组件为虚拟化改造做"子组件抽取复用"时（把单个列表项的渲染逻辑抽成 `SelectOptionItem`/`CascaderColumnItem`/`TreeSelectNodeItem` 独立函数组件，供虚拟化/非虚拟化两条渲染路径共用），描述这些子组件视觉样式的 CSS 规则（`.lotus-select-option`/`.lotus-cascader-item`/`.lotus-tree-select-node` 等）却仍然留在**主组件**（`Select`/`Cascader`/`TreeSelect`）的 `<style>` 块里没有跟着"搬家"。真机检查发现子组件渲染出的 DOM 元素上只有裸 class 名（如 `class="lotus-tree-select-node"`），完全没有被打上主组件的 scoped hash class（如 `tsrx-320e5cbf`），而主组件 `<style>` 块编译出的实际 CSS 选择器是 `.lotus-tree-select-node.tsrx-320e5cbf`（两个 class 同时命中才生效）——两者不匹配，规则形同虚设。TreeSelect 因为虚拟化列表用固定 `32px` 高度的包裹层定位每一项，而内容因为 `min-height:32px` 完全不生效、只剩内容自身 22px 的自然高度，点击命中位置系统性偏移到内容之外的空白区域，`e2e` 测试对"点击后浮层应关闭"的断言 100% 必现失败（`--repeat-each=10 --workers=1` 单 worker 串行 10/10 全部复现，排除环境噪声后确认是确定性 bug）；Select/Cascader 的对应子组件同样完全没有 padding/hover 背景色/字号等样式（`getComputedStyle` 实测 `padding:0px`，规范值应为 `6px 8px`），但因为现有 e2e 断言都是文本内容/交互结果而非视觉细节，长期未被测试抓到。根因经读 Ripple/tsrx 编译器源码确认（`~/i/ripple` 仓库 `packages/tsrx/src/transform/style-ref.js` 的 `create_style_class_map` 读取 `component.metadata.topScopedClasses`，其收集函数 `apply_css_definition_metadata`/`collect_css_prunable_elements` 只遍历 `node_children(component)`——严格限定在当前组件自身模板直接渲染的节点范围内），这是 tsrx 的既定 CSS 作用域设计，不是编译器 bug；Ripple 官方仓库的标准范式（`website-new/src/components/sidebar.tsrx`）里 `Sidebar`/`SidebarGroup`/`SidebarItem` 三层组件各自都有自己独立的 `<style>` 块，从未依赖父组件的 `<style>` 覆盖子组件内容。**修复方式**：给每个被抽取出来的子组件补上它自己独立的 `<style>` 块（内容从主组件 `<style>` 块里搬移过去，不是复制），子组件用 `@{...}` 模板语法时因为模板体出现两个顶层节点（渲染元素 + `<style>`）需要额外用 `<>...</>` fragment 包裹整个模板体。**通用教训**：(1) 任何"把渲染逻辑抽成独立子组件供多处复用"的重构，只要子组件渲染的元素带 class 名且样式定义留在原处（无论是主组件还是别的文件），必须显式核对这些样式是否在子组件自己的 `<style>` 作用域内——不能假设"class 名对得上就等于样式生效"；(2) 排查真机渲染尺寸与 CSS 声明值不符的问题，`getComputedStyle` 只能看到最终计算值，必须进一步遍历 `document.styleSheets` 找到实际命中的 `CSSRule.cssText`，对比其选择器（尤其是否含 scoped hash class）与目标元素的真实 `className`，才能区分"规则被更高优先级覆盖"和"规则选择器根本不匹配"这两种表面症状相似但根因完全不同的情况；(3) 一个功能改造如果在多个结构相似的组件间复制同一种模式（本例是"子组件抽取"这个模式被 Select/Cascader/TreeSelect 三次复用），某一次意外暴露的 bug 大概率也存在于其余几次里，只是被测试覆盖率的偶然差异掩盖——确认根因后要主动回查所有用了同一模式的组件，不能只修复最先暴露症状的那一个。
- **VChart（`@visactor/vchart`）构造函数 `new VChart(spec, options)` 的 `options.theme` 一旦传值，会被永久固化进实例的 `this._option.theme`，此后每次调用 `setCurrentThemeSync(newThemeName)` 切换主题都会被这个固化的旧值以最高优先级覆盖，视觉上表现为"主题切换 API 调用成功、参数正确，但画面完全没有变化"**（踩坑 #99，重大，Chart 组件首次实现真机验证暗色模式切换时发现，VChart 官方文档没有明确说明这个陷阱）：Chart 组件挂载时最初写的是 `new VChart(spec, { dom: container, theme: ensureLotusChartTheme() })`——直觉上"构造时传入初始主题"是最自然的写法，`typecheck`/单测/挂载后的首次截图全部正确（因为首次渲染的主题名和 `options.theme` 一致，不会触发覆盖）。真正暴露问题的是用 ego-browser 做"挂载后切换 `ConfigProvider.mode`"的端到端验证：点击切换到暗色后，用 canvas 像素采样确认背景色始终是挂载时的旧值（如 light 模式下錯误地卡在 `#ffffff`，切到 dark 后 `getImageData` 读到的仍是 `[255,255,255,255]` 而非预期的 `#16161a`），且期间 `chart.setCurrentThemeSync(...)` 确实被调用、传入的主题名和 `ThemeManager.registerTheme` 也确实生成了正确的深色主题对象——用 console 调试逐层确认"生成的主题数据是对的""方法调用发生了"之后，才把怀疑范围收窄到 VChart 内部 `_updateCurrentTheme` 这一步：读该方法源码（`node_modules` 里的真实 `.js` 而非 `.d.ts`，因为这个行为完全没有类型签名能体现）发现 `isEmpty(optionTheme) && isEmpty(specTheme)` 为 `true` 时才会直接用 `this._currentThemeName` 取新主题，否则会用 `mergeTheme({}, 旧主题, getThemeObject(optionTheme), ...)` 把 `optionTheme`（构造时那个固定字符串对应的主题对象）以最后展开、最高优先级的方式合并回去——`options.theme` 只要非空，这条"高优先级覆盖"分支就会一直生效，跟后续调用 `setCurrentThemeSync` 多少次都无关。**修复方式**：构造 `new VChart(spec, { dom: container })` 时不传 `theme` 字段，构造完成后立即显式调用一次 `chart.setCurrentThemeSync(ensureLotusChartTheme())` 应用初始主题——这样 `this._option.theme` 保持为空，`_updateCurrentTheme` 走的是"直接用 `_currentThemeName` 取值"的正确分支，之后任意次 `setCurrentThemeSync` 调用都能正常生效。**通用教训**：(1) 第三方库的构造参数如果同时提供"初始配置"的语义、又存在配套的运行时切换方法（`setXxx`/`updateXxx`），不要想当然认为两者是等价的两种用法（"构造时传初始值"与"构造后立刻调用一次 set 方法"），必须去读被调用库的实际源码确认构造参数是否会被内部逻辑当成"用户显式覆盖、优先级高于运行时切换"的信号而非单纯的"初始值"——这类语义差异几乎不会体现在 TypeScript 类型签名里，只能读 `.js` 实现；(2) 排查"API 调用了、参数也对、但视觉没变化"这类问题时，不要止步于确认调用链路本身正确（这次的 `ensureLotusChartTheme()`/`registerTheme` 全部正确），必须再往被调用方内部深挖一层，确认目标方法本身的实现逻辑是否存在会让新值被静默覆盖的分支；(3) 验证"运行时切换是否生效"这类问题不能只看无报错、返回值正确，要用能反映真实视觉结果的信号做验证（本次用 canvas `getImageData` 直接读像素 RGB 值，比读 DOM 属性或 JS 返回值更接近"用户真实看到什么"这个问题的本质）。
- **CSS Grid `repeat(N, 1fr)` 的轨道最小宽度默认是 `auto`（不是 `0`），当轨道内的子元素存在"由内容/第三方库运行时写入的固有尺寸"（不是由 CSS 显式约束的尺寸）时，子元素的固有尺寸会反过来撑大 `1fr` 轨道本身，若这个子元素还依赖 `ResizeObserver` 监听容器尺寸变化并回写自己的尺寸（VChart/图表库、部分富文本编辑器、iframe 等常见于此类库），会形成"内容撑大容器 → 容器变大触发观察者 → 观察者把内容设置得更宽"的正反馈循环，实测可以在几次重渲染内把轨道宽度从预期的几百像素撑到五位数像素**（踩坑，Chart 组件在 playground 3 列网格布局下用真机验证首次挂载与暗色模式切换时各触发一次，均实测复现）：playground 的 Chart demo 区块最初用 `gridTemplateColumns: 'repeat(3, 1fr)'`，Chart 组件内部的 VChart 实例首次挂载测量容器宽度的时机，与 grid 布局尚未完全收敛的中间状态发生竞态，写入了一个远超预期的 canvas 内联宽度（真机实测撑到 6000px+），这个过大的 canvas 又成为其所在 grid item 的固有内容尺寸，撑大了对应的 `1fr` 轨道；第一次尝试只在 Chart 组件自身的 `.lotus-chart` 容器上加 `overflow: hidden` 缓解了首次挂载的症状，但后续切换暗色主题触发 `updateSpecSync`/`setCurrentThemeSync` 引发的重新渲染，同样的正反馈再次复现——因为 `overflow:hidden` 只裁剪视觉溢出，并不能改变"grid item 在没有显式宽度上限时，`getBoundingClientRect`/`clientWidth` 会被子元素内容撑大"这个底层测量事实，加在 Chart 组件内部这层修复方式加错了层级（Chart 组件管不到、也不该管自己被外部容器用什么布局方式包裹）。**修复方式**：把 playground demo 的 `gridTemplateColumns` 从 `repeat(3, 1fr)` 改为 `repeat(3, minmax(0, 1fr))`——显式把轨道最小宽度从隐式 `auto` 改成 `0`，从根源上阻断"轨道被子元素内容撑大"这条链路，不依赖子组件自己加防御性 CSS。**通用教训**：(1) 任何要把"会自行测量并回写自身尺寸的第三方库组件"（图表、富文本编辑器、地图、视频播放器等）放进 CSS Grid 的 `1fr`/`auto` 轨道时，默认要用 `minmax(0, 1fr)` 而不是裸 `1fr`，这是搭配这类组件的标准姿势，不是本例独有的临时补丁；(2) 排查"容器被异常撑大"类问题时，要用 `getComputedStyle(grid).gridTemplateColumns` 直接读计算后的轨道宽度数值，而不是只看子元素或容器自身的 `getBoundingClientRect()`——本例的根因线索（三个轨道宽度全部是几千像素的异常值）只有查询 grid 容器本身的 computed style 才能直接看到，查子元素宽度只能看到"结果异常"，看不到"异常源头是轨道本身"这个更准确的定位；(3) 同一个正反馈类 bug 可能在"首次挂载"和"后续任意触发重渲染的操作"（本例是主题切换）两个不同时间点分别独立复现一次，只验证挂载时的效果不够，只要该组件存在会触发自身重新测量/重绘的运行时事件（resize、主题切换、数据更新等），都需要在真机验证里覆盖到这些后续事件发生后的状态，不能默认"挂载时验证通过就等于任何时候都不会复发"。
- **Rating（及代码库里其它 15+ 个组件复用的同一种"受控值 resync"实现模式）在受控模式下，若父组件的 `onChange` 选择不更新 `value` prop（这正是受控组件校验失败/业务规则拒绝变更的典型合法用法），组件内部会无条件把用户交互结果写入 `state`，且没有任何机制把这个被污染的 `state` 拉回受控值——UI 会永久停留在交互产生的中间态，真机验证过点击后 1 秒仍未被纠正**（踩坑 #100，重大且波及面广，本次全面质量复查工作流首次发现，且暴露了自动化 verify agent 本身可能给出错误论证的风险）：Rating 组件此前的实现是 `effect(() => { if (value === undefined) return; untrack(() => { state = {...state, value: value!}; }); })`——直觉上这个 effect"只要 value 变了就把 state 拉回来"，看起来能实现受控约束，`handleStarClick`/`handleKeyDown` 也确实无条件 `setState` 写入。审查工作流的一个 finder 正确指出了这个问题，但对应的对抗性 verify agent 给出了看似严谨、实则关键环节错误的推翻论证："state mutation triggers dependency re-evaluation"——即"改了 state 会让这个 effect 重新调度"，据此认为点击后 effect 会自动重新执行、把值纠正回去。**这个论证是错的**：该 effect 内部对 `state` 的读写全部包在 `untrack(...)` 里（`untrack(() => { state = {...state, value: value!} })`——箭头函数体内先展开 `...state` 再赋值，这个读取发生在 `untrack` 内部），`untrack` 期间的响应式读取根本不会被注册进 Ripple 运行时的依赖追踪（读 `~/i/ripple` 源码 `runtime.js` 的 `untrack()`/`is_block_dirty()`/`is_tracking_dirty()` 可确认：block 是否需要重新调度，只看它在运行时**非** untrack 包裹部分注册的依赖 `block.d` 是否变化）。这个 effect 唯一没有 untrack 包裹的读取是 `value` 这个 prop 本身，所以它**只在 `value` prop 发生变化时才会被重新调度**，不会因为 `state` 被组件内其它代码（如 `handleStarClick`）修改而自动重跑——若父组件的 `onChange` 决定不更新 `value`（`value` 引用/值都没变），这个 effect 永远不会再执行第二次，`state.value` 上残留的用户点击结果会一直显示下去。**这个错误推翻被发现的过程**：因为同一个 finder 维度在其它几个组件（Slider/ColorPicker/Cascader）上的类似发现也被以几乎相同的论证推翻，逐条核实这批"isControlled 相关"的 confirmed/refuted 判定时，没有直接采信 verify agent 的结论，而是亲自去读 Ripple 运行时源码求证"state mutation triggers dependency re-evaluation"这句关键论据是否成立，读代码即可证伪；进一步用 ego-browser 做真机验证（构造一个 `value=3, onChange={() => {}}` 的临时 demo，点击第 5 颗星），实测确认点击后视觉状态在 1 秒后依然停留在被污染的中间态（而非文档记录以为的"拉回 3"），坐实是真实 bug 而非"两种都合法的实现策略差异"。**修复方式**：对齐 Select 组件已验证正确的"写入时门控"模式——`RatingFoundation.handleClick`/`handleKeyDown` 新增 `isControlled` 参数，为 `true` 时不写 `value`（只写 `hoverValue`/`clearedValue` 这类没有对应 prop、纯本地交互态的字段），渲染层新增 `committedValue = isControlled ? value! : state.value` 派生值供 `displayValue` 使用，删除原来那个不可靠的 resync effect——受控模式下 UI 展示值永远直接来自 `value` prop 本身，不经过任何"事后纠正"的中间状态。**通用教训**：(1) "resync effect"（`effect(() => { if (propValue === undefined) return; untrack(() => syncStateFromProp()); })`）这个手法只能在"prop 变化"这一个事件源上生效，不能依赖它去纠正"组件内部其它代码修改了同一个 state"这类场景——这两种触发源根本不共享同一条依赖链，效果上是两回事，容易被表面的"看起来都是把 state 和 prop 对齐"这个意图误导成等价机制；(2) **自动化对抗性 verify agent 给出的论证即使读起来严谨、引用了具体代码行号，也可能在关键的框架底层行为细节上出错**——尤其涉及"某个响应式 effect 会不会被重新调度"这类需要真正吃透运行时依赖追踪机制才能准确判断的问题，人工复核时不能因为论证篇幅长、引用了源码就直接采信，同一个 finder 维度在多个相似组件上被同样理由反复推翻时，这本身就是一个值得警惕的信号（大概率是这批 verify agent 共享了同一个错误的框架行为假设，而不是碰巧这几个组件都设计正确）；(3) 验证"响应式副作用会不会在特定条件下重新触发"类问题，最终裁决权在真机行为而非代码阅读推理——本次是先读 Ripple 源码定位到 `untrack` 阻断依赖追踪这个精确机制，再用 ego-browser 构造"父组件拒绝更新"的场景实测复现，两者缺一不可（只读源码可能忽略某个隐藏的重新调度路径，只做真机验证若没构造对场景也可能恰好看不出问题）；(4) 这类"resync effect"模式在代码库里被至少 15+ 个组件复用（Slider/ColorPicker/Cascader/TreeSelect/DatePicker/Transfer/TagInput/AutoComplete 等），本次经用户明确决定只修复 Rating、控制改动范围，其余组件是否存在同一缺陷、缺陷严重度是否因组件交互特性不同而有差异，留待后续任务专项排查，不要默认"读到了同一段代码模式就等于所有实例都是同一优先级的 bug"——不同组件的用户可感知影响程度可能差异很大。
- **一条错误的技术假设写进单个组件的代码注释后，经"复制这段代码/这个模式到新组件"这个开发方式，扩散成了至少 12 处（10+ 个组件文件）同一形状的真实资源泄漏——document/window 级常驻事件监听器（点击外部关闭浮层的 mousedown、拖拽类组件的 mousemove/mouseup、Esc 键盘监听）在 `effect()` 里注册却从未返回清理函数，组件卸载后监听器永久残留，闭包持有已失效的组件状态引用**（踩坑 #101，重大且波及面广，本次全面质量复查工作流发现，后续系统性排查确认影响范围比 finder 报告的更大）：`packages/ripple/src/input/select/index.tsrx` 第 247 行的代码注释原文是"Ripple 的 effect() 没有返回清理函数的机制（不同于 React useEffect 的 cleanup），故不在 effect 里装卸监听器，而是组件挂载时装一次常驻的 document 级监听"——这句话在事实层面是错的（`effect()` 完整支持 `return () => {...}` 形式的 teardown，代码库内 `Modal`/`HotKeys`/`DragMove`/`SideSheet`/`UserGuide`/`Sidebar.Container`/`Cropper` 等组件都有正确使用这个机制的先例，且时间上早于 Select 这段注释被写下），但因为 Select 是这类"点击外部关闭浮层"交互模式里较早实现、后续被反复参照复制的一个组件，Cascader/TreeSelect/AutoComplete/DatePicker/TimePicker/ColorPicker（两处）在实现同一交互模式时都原样复制了"不写 cleanup"这个做法（部分文件甚至留了"同 Select"/"同 Cascader"的注释明确承认是照抄），Slider/TagInput/Transfer/ColorPicker（另一处）的拖拽类 mousemove/mouseup 监听虽然没有照抄那条错误注释，但同样遗漏了 cleanup，形成了同一缺陷模式独立发生了多次的局面；额外系统性排查（写脚本扫描全部 `effect(() => {...})` 块，检查内部含 `addEventListener` 调用的块是否在闭合前有配对的 `return () => {...removeEventListener...}`）还发现了 finder 报告之外的 3 处遗漏——`Cropper`（拖拽裁切框/手柄的 mousemove/mouseup）、`Image.preview`（Esc 键盘监听 + 拖拽平移的 mousemove/mouseup）——证明人工/AI finder 逐文件审查即使覆盖了大部分范围，也可能因为审查批次划分或注意力局限漏掉个别实例，全量代码库最终要用可复现的脚本扫描而非仅凭审查报告的清单收尾。**修复方式**：全部 12 处统一补上与 `document.addEventListener`/`window.addEventListener` 一一对应的清理调用（`return () => { document/window.removeEventListener(type, handler); ... }`），同时删除 Select 组件里那条错误的技术假设注释。**通用教训**：(1) 代码注释里对底层框架/运行时行为的断言性描述（"XX 没有 YY 机制"这类否定式论断）如果本身是错的，会通过"后来者直接照抄这段代码/这个模式，连带把错误理由也一并复制"这条路径持续扩散伤害范围，比一处孤立的 bug 更难根除——写这类论断前最好先去实际验证（哪怕只是找一个代码库里其它地方的反例），而不是凭直觉或印象下结论；(2) 排查"某个模式是否在代码库里被复制传播"类问题，同批次审查里出现"同一个 finder 维度、多个相似组件、几乎相同的措辞"这个信号本身就值得深挖，无论最终判定是"这些发现全部真实"还是"这些发现全部因同一个错误假设被误判"，两种情况都指向"存在一个跨组件的共享成因"而不是若干个独立巧合；(3) 修复"审查发现的问题清单"时，不要止步于逐条修复清单上列出的条目——用能自动化复现的检查手段（哪怕是几行正则脚本）把同一缺陷模式的检测范围跑一遍全代码库，弥补人工/AI 审查依赖"注意力覆盖到了哪里"这个不可靠的前提，本次正是靠这一步补上了 finder 报告遗漏的 3 处。
- **Chart 组件的图表分类色板（`--lotus-color-data-0~19`）虽然渲染层 `theme.ts` 早已写好消费逻辑（`dataSchemeTokenMap`），但 `packages/tokens` 从未真正定义过这 20 个 CSS 变量，导致多系列/多分类图表配色一直静默回退到 VChart 自带默认色板，从未真正用上 lotus 品牌色——首次实现修复时，又用了错误的数据来源（`@visactor/vchart-semi-theme` npm 包自己的兼容兜底值），把"亮暗模式共用同一套颜色"这个错误结论当成了既定事实**（Chart 组件能力扩展任务的后续排期项，AGENTS.md 规定的一手来源与临时取用的二手来源给出了互相矛盾的答案，来源优先级判断错了会产出完全走样的实现）：`specs/cross-cutting/theme-tokens.spec.md` 早就把这批 token 标记为"暂缓，与 Chart/Semi DV 待裁定项绑定"，但 Chart 组件交付后这个绑定条件其实已经解除，`theme.ts` 里 `dataSchemeTokenMap` 数组已经列出了 20 个 `--lotus-color-data-N` 变量名准备被 VChart 的 `getTokenValue()` 读取，`packages/tokens` 那边却从未跟进定义——这是"改了消费方代码、没有同步改源头 token 定义"的一个典型例子，且因为 `getComputedStyle().getPropertyValue()` 对不存在的 CSS 变量返回空字符串而非报错，`|| defaultValue` 会静默回退到 VChart 内置色板，不会有任何运行时报错或 typecheck 失败提示这个缺口，只有真机检查 canvas 实际渲染颜色才能发现。**取值来源踩的坑**：着手补齐这批 token 时，第一直觉是去 npm 上 pack 官方 `@visactor/vchart-semi-theme` 包查真实色值（因为 `theme.ts` 的实现本身就是照抄这个包的接入方式），从中读到的 `common/data-scheme.js` 是一个硬编码的 20 色数组，被 `light/color-scheme.js` 和 `dark/color-scheme.js` **同时引用**，看起来像是"官方也认为亮暗该共用一套数据色"，据此得出结论并告知用户确认。但按 AGENTS.md 规定的一手来源优先级（`specs/cross-cutting/theme-tokens.spec.md` 与既有 `static-tokens.ts` 注释都明确指向本地 `~/i/semi-design/packages/semi-theme-default/scss/{global.scss,variables.scss}` 是权威来源，其余都是"摘要"或衍生实现），重新去查这份 scss 源文件，发现 `:root`（亮色）块和 `body[theme-mode="dark"]` 块**各自独立定义了完全不同的 20 个 `--semi-color-data-N` 值**——`vchart-semi-theme` 包里那份"共用"的实现，只是 VChart 主题层自己简化的兼容兜底逻辑（这个包的职责是把 CSS 变量名接入 VChart 的 `getTokenValue()` 读取机制，不是复刻 Semi 应用真实用的每一个具体色值），不代表 Semi 应用真实的暗色数据色板长什么样。**修复方式**：`chartDataColor` 从 `string[]`（20 色，被同时用于亮暗两个 CSS 块）改为 `Record<'light'|'dark', string[]>`（各自独立的两套 20 色，共 40 个真实色值，从一手来源 `global.scss` 逐条核对转换），`build-css.ts` 的 `chartDataLines()` 从无参函数改为接受 `mode` 参数，分别在亮/暗两个 `:root` CSS 块调用时传入对应模式；RGB→hex 的手工换算用 Python 脚本批量计算校验，不靠肉眼心算（40 个颜色逐个手算极易出错）。**通用教训**：(1) "改了消费方代码、期望某个 token 存在"和"这个 token 真的已经定义"是两件独立的事，`grep` 确认 `packages/tokens` 里是否真的存在对应定义，比读消费方代码里的注释更可靠——本例 `theme.ts` 的注释"对应 `packages/tokens` 新增的 `--lotus-color-data-0..19`"读起来像是既成事实，实际上是超前于实现的规划性描述，没有人回头补上真正的定义；(2) `getComputedStyle().getPropertyValue()` 对不存在的 CSS 变量静默返回空字符串，配合 `value || defaultValue` 这类兜底写法，会让"token 缺失"和"token 存在但故意想用默认值"这两种情况在运行时完全不可区分，排查这类问题必须用真机检查实际渲染结果（如本例读 canvas 像素色值），不能依赖 typecheck 或常规功能性测试——两者都不会因为一个 CSS 变量没定义而报错；(3) 遇到"一手来源"和"网上/npm 包里能查到的二手实现"给出矛盾信息时，二手来源即使是官方包也可能只是该场景下的简化/兼容实现，不能默认它完整复刻了一手来源的每一个细节——本例的 `vchart-semi-theme` 包对"亮暗模式的数据色板是否需要独立"这件事的处理，就与 Semi 应用真实的 CSS 变量定义不一致，只有真正去读一手来源的源文件才能发现这个差异，读第三方包的 JS 实现细节不能替代读权威来源本身。
- **`Spin` 组件包裹 `children` 时不会把父容器的高度传递给内容区——`.lotus-spin-block`（`children` 非空时套用的 class）只有 `width:100%`，没有 `height:100%`，`.lotus-spin-children` 这层内部包裹 `div` 更是完全没有高度相关样式，把 `Spin` 当"撑满容器的加载态包装层"直接套在一个需要固定尺寸的第三方库容器（Chart 的 VChart canvas）外面，被包裹内容的 `height:100%` 因为找不到有效的高度基准而被浏览器忽略，退化成内容自身的默认高度**（Chart 组件能力扩展新增 loading 态时发现，真机验证 boundingBox 高度与预期不符）：Chart 组件最初实现是 `<Spin spinning={loading} style={{width:'100%',height:'100%'}}><div class="lotus-chart" style={...}/></Spin>`，只在 `Spin` 根节点补了内联 `height:100%`，以为这样就能让高度沿着 DOM 树逐层传递下去——但 `Spin` 内部还夹了一层 `.lotus-spin-children`（`children != null` 时才渲染的包裹层），这层 `div` 没有高度样式，导致 `.lotus-chart` 的 `height:100%` 找不到基准，实测 canvas 高度回退到 VChart 默认的 500px（demo 容器实际是 260px）。用 ego-browser 逐层打印 `getBoundingClientRect()` 才定位到具体是哪一层断链——`bar`(260px 正确) → `wrapper`(260px 正确) → `spin`(260px 正确，因为直接给它加了内联 style) → **`children`(500px，断链点)** → `chart`(500px，被上一层带偏)。**修复方式**：`Spin` 组件本身暴露了 `childStyle` prop（用途正是"给 `.lotus-spin-children` 这层内部包裹 div 传自定义样式"，此前一直没被用到过这个特性），Chart 组件同时传 `style={{width:'100%',height:'100%'}}` 和 `childStyle={{width:'100%',height:'100%'}}`，让根节点和内部包裹层两层都拿到显式高度，断链问题消失；同时发现 Chart 自己的 `.lotus-chart` CSS 规则本身也遗漏了 `width:100%;height:100%`（此前 `width`/`height` 只写在外层新增的 `.lotus-chart-wrapper` 上，忘记同步给内层元素——这是本次重构过程中自己引入的第二个疏漏，不是 `Spin` 组件的问题），两处一起补齐后高度才完全正确。**通用教训**：(1) 复用一个"包裹 children 提供某种蒙层效果"的组件（`Spin`/`Popover`/`Tooltip` 这类）时，不能假设它会像最简单的 `<div style="height:100%">{children}</div>` 那样把高度约束原封不动传下去——这类组件为了实现自己的效果（loading 指示器叠加、多状态切换动画等）往往会引入额外的包裹层，每一层是否传递高度/宽度都要单独确认，最快的确认方式是用真机 DOM 检查工具逐层打印 `getBoundingClientRect()`，而不是读源码猜测层级关系是否透明；(2) 组件暴露的"次要 style prop"（如 `Spin` 的 `childStyle`，区别于最外层的 `style`）往往正是为了解决这类"内部有多层包裹、外层 style 传不到内层"的问题而设计的，遇到"最外层 style 好像没生效"的现象时，先检查组件是否已经提供了针对内部特定层级的 style 入口，而不是自己在外部叠加更多层 wrapper 硬顶；(3) 重构一个已有组件的渲染结构（本例是给 `.lotus-chart` 外面新增一层 `.lotus-chart-wrapper` 来承载 loading/empty/导出按钮的定位需要）时，原来直接写在旧的最外层元素上的 CSS 规则（`width:100%;height:100%`）必须显式确认是否需要跟着挪到新的实际渲染元素上，不能想当然认为"反正是同一个语义位置，样式复制过去了就行"——本例恰好选择性地把 `width/height` 只留在了新外层，忘了原来的元素自己也需要，这类"重构时旧样式规则该分给哪一层"的疏漏在改造涉及多层嵌套的组件时很容易发生。
- **Select/Cascader/TreeSelect 三个 combobox 组件此前完全没有键盘导航实现——DOM 上有 `role="combobox"`/`aria-expanded`，但键盘用户除了原生 Tab 聚焦以外无法用 ArrowDown 展开面板、无法在选项间移动、无法用 Enter 选中、甚至 Esc 都关不掉面板**（踩坑 #105，Class C 剩余部分，是本次键盘无障碍补齐里工作量和设计复杂度最高的一批，采用统一的 `aria-activedescendant` 虚拟焦点模式实现）：三个组件都用 `<Popover trigger="custom" visible={isOpen} closeOnEsc={true}>` 完全接管开关状态，但 `Popover` 内部 `handleKeyDown` 的 Esc 处理是 `if (closeOnEsc && !isControlled && event.key === 'Escape' ...)`——`trigger="custom"` 模式下 `visible` prop 存在，`isControlled` 恒为 `true`，这个条件短路后 `closeOnEsc={true}` 这个 prop 形同虚设，三个组件都误以为"我传了 closeOnEsc 就该有 Esc"，实际上从未生效过；除此之外三个组件的 trigger 元素上一直没有绑定任何 `onKeyDown`，ArrowDown/ArrowUp/Enter 全部没有处理。**设计方案**：DOM 焦点始终留在 trigger（`role="combobox"`）上，不做浏览器原生的焦点转移到选项——这是 WAI-ARIA combobox 模式推荐的标准做法，用一个"虚拟焦点索引"（`activeIndex`/`activeCoord`）记录当前高亮到哪一项，配合 `aria-activedescendant` 属性告知屏幕阅读器、配合 CSS class 给出视觉高亮，ArrowDown/ArrowUp 移动这个索引（自动跳过 `disabled` 项），Enter 触发选中，Escape 关闭面板，未打开时按方向键直接展开并高亮首项。三个组件的数据结构差异导致"索引"的具体形态完全不同：Select 是扁平的 `optionList`，索引是单个数字；TreeSelect 复用了已有的 `flatNodes`（按当前展开状态摊平的一维可见节点数组，`FlatTreeNode` 自带 `disabled`/`isLeaf`/`hasChildren` 字段），索引同样是单个数字，额外要处理 ArrowRight/ArrowLeft 展开收起节点（复用已有的 `handleToggleExpand`）；Cascader 是真正的多列结构（`columns: CascaderEntity[][]`），索引必须是 `{col, row}` 二维坐标，ArrowRight 展开当前行进入下一列（复用 `handleItemActivate`）、ArrowLeft 回到上一列、Enter 复用现有的 `handleColumnItemClick` 点击语义分派（叶子节点或 `changeOnSelect` 时选中，否则展开）——三处都不是简单复制同一份代码，而是把同一个交互意图（方向移动/展开/选中/关闭）套进各自已有的数据结构和现有处理函数上。Cascader 排查中额外发现一个必须避免混淆的陷阱：`CascaderColumnItem` 已经有一个 `active` prop，但它读的是 `state.activeKeys.has(entity.key)`——这个字段的语义是"这个节点在当前鼠标/点击展开出的路径上"（用来决定要不要渲染下一列），跟"键盘当前聚焦在哪一行"是完全不同的两个概念，如果直接复用这个 prop 会导致键盘导航状态和鼠标交互状态相互脏写，必须新增一个独立的 `keyboardActive` prop 和独立的 CSS class。**修复方式**：三个组件的 trigger 元素统一补 `onKeyDown`（ArrowDown/ArrowUp/Enter/Escape，Cascader/TreeSelect 额外加 ArrowRight/ArrowLeft）与 `aria-activedescendant`；每个选项/节点/级联项都生成一个基于组件实例 id（`Math.random().toString(36).slice(2)`，对齐 `UserGuide` 组件已有的一次性 id 生成模式）+ 位置索引拼出的稳定 DOM id，供 `aria-activedescendant` 指向；高亮态通过新增的 `active`（Select 复用这个名字但语义单一不冲突）/`keyboardActive`（Cascader，避免与既有 `active` prop 冲突）prop 驱动渲染，不占用/复用任何已有的、语义不同的状态字段。**排查中意外撞见的 tsrx 编译器踩坑**：给 TreeSelect 写 `firstEnabledNodeIndex`/`nextEnabledNodeIndex` 两个辅助函数时，最初把它们定义在 `let &[flatNodes] = track(...)` 声明**之前**（因为想放在其它状态变量声明的集中区域），`tsrx-tsc` 报错 `Accessing a tracked object directly is not allowed`——虽然 JS 函数声明会被提升、运行时调用时机 `flatNodes` 早已存在且赋值完毕，但 tsrx 编译器做的是静态词法分析，它似乎要求"引用某个 `track()` 变量的函数，其定义位置必须在该变量声明之后"才能正确识别这是一次合法的懒解构读取，把这两个函数挪到 `flatNodes` 声明之后问题立即消失；Cascader 实现时已经吸取这个教训，从一开始就把 `firstEnabledRow`/`nextEnabledRow` 放在 `columns` 声明之后，一次 typecheck 就通过，没有重蹈覆辙。**通用教训**：(1) 组件内部转发某个第三方/底层封装组件（这里是 `Popover`）的某个 prop（`closeOnEsc`）时，必须验证这个 prop 在自己实际使用的模式（`trigger="custom"`）下是否真的会生效，不能因为"我传了这个 prop"就假设功能已经实现——`Popover` 的 `closeOnEsc` 文档意图是服务非受控场景，`trigger="custom"` 恰好是受控场景，两者语义上互斥但类型系统和运行时都不会报错提示这一点，只有读到 `!isControlled` 这行具体判断代码才能发现；(2) `aria-activedescendant` 模式下"虚拟焦点索引"的数据结构必须贴合组件本身已有的数据模型，不能强行套用同一份实现在结构不同的组件间复制——扁平列表、树形摊平数组、二维多列各自需要不同形状的索引和不同的移动/展开逻辑，生搬硬套只会让代码难以理解且容易出错；(3) 一个已存在的 prop 名字（如 `active`）如果在不同组件间或同一组件内被用来表达不同语义（"这个节点在当前展开路径上" vs "键盘当前聚焦在这里"），新增功能时要先确认清楚这个已有语义的真实所指，不能望文生义假设可以直接复用，必须新增一个语义单一、命名明确的独立字段；(4) tsrx 编译器对"函数定义与其引用的 `track()` 变量的相对声明顺序"有静态检查要求，写辅助函数时养成"在依赖的响应式变量声明之后立即定义相关辅助函数"的顺序习惯，可以从源头避免这类编译错误，不需要每次都靠报错倒推。
- **一批分散在不同组件的键盘无障碍缺口：InputNumber 步进按钮只绑 onClick 不支持方向键、DatePicker 面板无 Esc 关闭路径、Resizable 8 个调整手柄纯靠 onMouseDown 触发对键盘用户完全不可达、HotKeys 提示徽标传了 onClick 时却没有 role="button"/键盘等效触发**（踩坑 #104，Class C 部分修复，Select/Cascader/TreeSelect 的 combobox 方向键导航因涉及从零实现"当前高亮索引"状态机、風险與工作量都明显更大，留在同一 Class C 范围内单独处理）：InputNumber 的 `handleStepUp`/`handleStepDown` 已经存在且被加减按钮的 onClick 复用，只是 `<input>` 本身没有 `onKeyDown` 把 ArrowUp/ArrowDown 接到同一对函数上——原生 `<input type="number">` 有方向键步进的默认行为，但 lotus InputNumber 是 `type="text"` 的自定义控件，浏览器不会替开发者补上这个交互，必须显式实现。DatePicker 有 `closePanel()` 函数和"点击外部关闭"的 `document` 级 mousedown 监听，但没有对称的 Esc 处理——这与 Popconfirm 此前踩过的坑（已修复）是同一形状的缺口，只是出现在不同组件。Resizable 的 8 个方向手柄 `<div>` 完全没有 `role`/`tabIndex`/`aria-label`/`onKeyDown`，键盘用户无法感知这些手柄的存在，更谈不上操作；较难的部分是"键盘按方向键调整尺寸"没有连续指针坐标可用，不能直接复用基于 `pointerX - startX` 增量计算的 `onResize` 方法，需要在 Foundation 新增一个 `resizeByStep(deltaWidth, deltaHeight, constraints)`——以固定步长直接对当前 `state.width/height` 做一次性加减，且这个 delta 的符号要按手柄方向换算（如 `left` 手柄按 ArrowLeft 意味着手柄往外移、容器变宽，对应 `deltaWidth` 为正，与 `right` 手柄按 ArrowRight 语义相反但产生同方向的宽度变化）。HotKeys 是"快捷键提示徽标"，`onHotKey` 才是真正响应真实按键组合的路径，`onClick` 只是给鼠标用户的额外便利点击——但只要传了 `onClick`，这个渲染出来的 `<div>` 就成了一个可交互控件，必须有 `role="button"`/`tabIndex={0}`/`aria-label`（此前完全没有暴露这个 prop）/Enter 或 Space 的键盘等效触发，不传 `onClick` 时则保持纯展示语义（不应该被无意义地推入 Tab 序列）。**修复方式**：InputNumber 在 `<input>` 上加 `onKeyDown`，ArrowUp/ArrowDown 分别调用已有的 `handleStepUp`/`handleStepDown`；DatePicker 新增一个 `window` 级 Esc 监听 effect（对齐 Popconfirm 已验证的模式，含清理函数）；Resizable 手柄补齐 `role="button"`/`tabIndex={0}`/`aria-label`（新增 `Resizable.handleLabel(direction)` locale 字段，中英文各自枚举 8 个方向的描述文案）/`onKeyDown`（按手柄方向换算方向键符号，调用新增的 `resizeByStep`）；HotKeys 新增 `'aria-label'` prop，仅在 `onClick` 存在时给容器加 `role="button"`/`tabIndex={0}`/`aria-label`，并加一个 Enter/Space 的 `onKeyDown` 处理函数等效触发 `onClick`。**通用教训**：(1) "只绑 onClick/onMouseDown、没有对应键盘路径"这类缺口不会被 typecheck、常规功能性 e2e 测试甚至视觉走查发现——因为鼠标交互路径完全正常，只有专门用 `page.keyboard`/`.press()` 模拟键盘操作的测试才能捕捉到，这是这批 finding 集中出现在"事后补齐无障碍"这个专项任务里而非日常开发中被发现的原因；(2) 键盘等效实现不能机械照搬"调用同一个函数"就完事——Resizable 手柄的方向键步进就是一个反例，鼠标路径的核心算法（`onResize`）是基于连续坐标增量设计的，键盘路径没有这个前提条件，必须在 Foundation 层补一个专门服务键盘交互、语义不同但复用同一批约束校验逻辑（`clamp`/`lockAspectRatio`）的新方法，不能强行把两种输入模式的算法揉进同一个函数签名；(3) 一个"传了某个可选 prop 才具备可交互语义"的组件（如 HotKeys 的 `onClick`），它的 ARIA role/tabIndex/键盘处理都应该跟着这个 prop 是否存在而条件性启用，不能无脑常驻——否则纯展示用法（不传 `onClick`）会被意外地推进 Tab 序列，造成键盘用户导航体验的噪声。
- **Foundation opts（或等价的配置闭包变量）挂载时固化、不响应 prop 运行时变化，这个缺陷模式独立出现在至少 6 个组件里，其中两个（IconButton/FloatButton）是与已知踩坑 #94 完全同根因、但没有跟着 Button 组件本身一起修的遗漏实例，另外三个（PinCode/TagInput/ColorPicker）是构造 Foundation 时传入的 `opts` 对象只在挂载时求值一次、Resizable 是常驻事件监听闭包捕获了挂载时的约束值**（踩坑 #103，Class A 批量修复，其中 ColorPicker 排查过程中意外发现了一个与 Rating/Cascader（踩坑 #100）同根因、但不在原始 finding 清单里的独立受控写入门控缺失）：`packages/ripple/src/basic/icon-button/index.tsrx`/`float-button/index.tsrx` 两处都用 `let &[state] = track<ButtonState>({ disabled, loading })`（非函数字面量，只在挂载时对 `{disabled, loading}` 求值一次）构造 `getState`，而 `Button` 组件本身早已按踩坑 #94 的教训改成 `getState: () => ({ disabled, loading })`（实时读 prop）——IconButton/FloatButton 复用同一个 `ButtonFoundation`/`FloatButtonFoundation`，却没有同步这次修复，运行时 `disabled` 从 `true` 变 `false` 后点击回调依然被永久拦截，这是"同一个 bug 被证实存在于消费某个共享 Foundation 的其中一个组件后，没有推广检查其余消费方"的典型遗漏（与 Class D 错误注释扩散、Class B resync effect 模式扩散是同一类教训——一个 bug 一旦确认是"某个 Foundation/模式的通病"，必须列出全部消费方逐一核查，不能只满足于修复最先暴露症状的那个）。PinCode 的 `{ count, format }`、TagInput 的 `{ separator, allowDuplicates, max }`、ColorPicker 的 `{ alpha }` 都是构造 `new XxxFoundation(adapter, opts)` 时传入的第二个参数对象，Foundation 内部方法（`writeChar`/`handlePaste`、`addTagsFromInput`、`applyHsva`/`handleHexInput` 等）大量直接读 `this.opts.xxx` 做校验/裁剪/边界判断，这些配置 prop 一旦运行时变化（切换验证码位数、动态调整标签数量上限、切换是否支持透明度通道），Foundation 内部逻辑永远沿用挂载时刻的旧值，是真实的功能性 bug 而不只是显示问题；Resizable 的 `minWidth`/`maxWidth` 等约束更隐蔽——它们被拼成一个 `const constraints` 对象，被"只在挂载时创建一次、之后不重新调度"的常驻 `document` 级 `mousemove` 监听闭包捕获，即便把 `constraints` 改成 `track()` 响应式变量也无济于事（这个 effect 本身不会因为响应式依赖变化而重新执行，闭包内旧的 `constraints` 引用不会被替换）。**修复方式**：IconButton/FloatButton 对齐 Button 已验证的模式，`getState` 直接实时返回 `{ disabled, loading }`（或 `{ disabled }`）、`setState` 留空；PinCode/TagInput/ColorPicker 把 `opts` 从"传入即固化"改为"保留同一个对象引用，用一个 `effect(() => { foundationOpts.xxx = xxx; })` 在每次 prop 变化时原地更新其字段"（对齐 Slider RTL opts 固化踩坑 #98 已验证的修复手法：Foundation 构造函数里 `this.opts = opts` 保存的是引用而非深拷贝，修改这个共享引用的属性即可让下一次方法调用读到新值，不需要重新构造 Foundation 实例）；Resizable 则是在 `handlePointerMove` 回调内部每次移动都重新读取 `minWidth`/`maxWidth` 等 prop 现场构造 `constraints`，而不是依赖任何外层缓存的对象。ColorPicker 排查过程中额外发现的独立问题：`applyHsva`/`handleRgbaChannelInput`/`handleHexInput` 等全部写方法此前无条件 `this.setState({ value })`，且组件层有一个和 Rating 修复前完全同构的 resync effect（`effect(() => { if (value === undefined) return; untrack(() => { state = {...state, value: value!}; }); })`）——按 Class B 已验证的"写入时门控"模式补上 `isControlled` 参数，受控时不写 `state.value`，只返回计算结果供调用方拼装 `onChange` 载荷。**通用教训**：(1) 排查"opts/配置闭包固化"这类模式时，必须先问一句"这个配置在真实使用场景里是否可能在组件已挂载、仍在交互时发生变化"（对齐踩坑 #98 已有的判断标准），但同样重要的是要检查"这个 Foundation 类是否被多个组件复用"——如果是，一旦在某一个消费方确认了修复方式，必须列出 `grep` 出的全部消费方逐一核查是否需要同步修复，不能默认"这个 Foundation 的 bug 已经在某处修过了"就跳过其余复用点；(2) 不是所有"配置值固化"的缺陷都能用同一种修复手法——`opts` 对象作为独立构造参数传入时，"保留引用、effect 原地更新字段"是最小改动的正确方案，但如果配置值是被一个"只创建一次、不重新调度"的事件监听闭包直接捕获（而非通过共享的 `opts` 对象引用间接访问），responsive 化这个变量本身没有意义，必须让消费该值的回调在每次真正执行时重新读取最新的 prop 值；(3) 修复一批"同类 finding"时，读代码顺手发现的、不在原始清单里的新问题（无论是同根因的独立实例，还是完全不同类别的缺陷）都要如实记录并评估是否本次一并处理，不能因为"任务范围是 Class A"就假装没看见——本次 ColorPicker 的受控门控缺失就是这样被发现的，如果严格拘泥于"只做 opts 固化"，这个真实 bug 会被无限期搁置。
- **Cascader/TreeSelect/Upload 三个组件复用了与 Rating（踩坑 #100）同一根因的"受控值写入未门控"模式，比对角 Select 已验证的"写入时门控"修复模板逐一补齐，同时在这批修复的真机验证过程中意外发现了一个全新的、与 `isControlled` 门控完全无关的独立 bug——TagInput 复用 `Tag` 组件的 `closable` 能力时，`Tag` 自身携带的非受控 `internalVisible` 本地状态会在点击关闭按钮时无条件自我隐藏，与 TagInput 父级"是否真的移除这个标签"的受控判断产生了两套独立的显示决策**（踩坑 #102，Class B 批量修复，`isControlled` 门控部分对齐已知模式，Tag 嵌套冲突部分是本次真机验证意外发现的新缺陷类型）：Cascader 的 `handleSingleSelect`/`handleMultipleCheck`、TreeSelect 的 `handleSelect`/`handleCheck`、Upload 的 `addFiles`/`patchFile`/`remove`/`replace`/`clear`/`applyBeforeUploadResult` 全部新增 `isControlled` 参数，受控时不写对应字段（Cascader 的 `selectedKey`/`checkedKeys`，TreeSelect 的 `selectedKey`/`checkedKeys`/`independentCheckedKeys`，Upload 的 `fileList`），只返回"如果这次操作被接受，结果会是什么"供 `.tsrx` 层拼装 `onChange` 载荷；Upload 因为 `emitChange()` 原来统一读 `state.fileList` 兜底，受控门控生效后这个读取会拿到操作之前的旧值，一并把 `emitChange` 改造为显式接收调用方算出的新 `fileList` 参数，`patchFile`/`applyBeforeUploadResult` 等原来只返回单个 `FileItem` 的方法同步扩展为返回 `{fileList, item}` 提供完整列表。三个组件的 Foundation 单测及 e2e 回归测试均按"受控模式下操作不改变 state，但返回值仍带完整新结果"的断言模式补齐，ego-browser 真机验证（受控值 + `onChange={() => {}}` 场景下点击/清除操作后 UI 保持不变）全部通过。**TagInput 的独立发现**：按同一根因假设，原以为 TagInput 也需要同样的门控（Foundation 侧 `removeTagAt`/`removeLastTag`/`clearAll`/`moveTag` 实际上早就正确门控了，属于本来就对齐好的组件），但真机验证"点击标签关闭按钮"这条路径时，退格键删除测试正常通过、点击关闭按钮测试却稳定复现"标签消失"——DOM 检查发现真相不是 `state.tagsArray` 被写坏（长度仍是 2），而是 `<Tag closable onClose={...}>` 渲染出的第一个标签自己变成了空壳（内部全部 `@if` 分支判定为 false）：`Tag` 组件自身实现了一套独立的非受控可见性状态（`let &[internalVisible] = track<boolean>(true)`，`handleClose` 在没有收到外部 `visible` prop 时会无条件 `internalVisible = false` 让自己消失，再触发 `onClose` 回调），TagInput 调用 `<Tag>` 时没有传 `visible` prop，Tag 就按照自己的默认"点击关闭即消失"语义自治运行——这与 TagInput 父级"标签是否显示完全由 `state.tagsArray`/`visibleTags` 决定"的设计意图产生了两套独立、互不知情的显示状态源，受控模式下 TagInput 因门控生效正确没有从列表移除这个标签，但 Tag 自己已经把自己隐藏了，视觉上呈现出与 Cascader/TreeSelect/Upload 表面相似的"UI 被污染"症状，根因却完全不同。**修复方式**：TagInput 渲染 `<Tag>` 时显式传入 `visible={true}`，强制 Tag 进入受控可见性模式（`isControlledVisible` 恒为 `true`），Tag 自身永远不会因为点击关闭而自我隐藏，标签的显示与否完全交由 TagInput 基于自己的 `state`/`visibleTags` 决定，`Tag` 组件在其它独立使用场景（如 `Tabs`/`Modal` 自身实现的 closable，均未复用 Tag 组件）不受影响。**通用教训**：(1) 修复一批"同一根因 finding"时，不能假设"读代码确认这批组件用了同一段问题代码"就等于"修复方案对每个组件都同样适用、且这批组件不会再有别的问题"——TagInput 这次证明了即使 Foundation 层的 `isControlled` 门控完全正确，只要复合组件内部复用了另一个自带独立状态管理的子组件（尤其是这个子组件本身设计成"开箱即用、默认非受控自治"），仍然可能在完全不同的层级上产生外观相似的 bug，真机验证不能因为"这个组件的门控代码看起来没问题"就跳过，退格键场景恰好绕开了 Tag 组件的 `onClose` 路径，只测这一种交互方式会让这个 bug 彻底漏网；(2) 复用一个自带内部状态的展示型组件（尤其是"可关闭/可隐藏"这类天然有开关语义的组件）作为更大复合组件的构建块时，如果父组件想要完全接管"这个东西是否显示"的决策权，必须显式把子组件推入受控模式（传入对应的 `visible`/`checked`/`open` 等 prop 锁定这条状态通路），不能依赖"子组件默认行为恰好和父组件预期一致"这种隐式假设——两边各自维护一份"是否显示"的状态源，只要触发路径没有完全对齐（如本例的点击关闭 vs 退格删除），迟早会在其中一条路径上暴露不一致；(3) 同一批"回归防护测试"要覆盖同一个操作意图的全部实现路径，不能因为其中一条路径（退格键）测试通过就推断另一条路径（点击关闭按钮）必然也没问题——两条路径尽管产品语义相同（都是删除标签），底层调用链完全不同（一个是纯 Foundation 方法调用，另一个额外途径了一个有自己内部状态的子组件），必须逐条路径分别验证。
- **AiChatDialogue 的流式消息列表容器常驻 `aria-live="polite"`，实现时看起来是"标准无障碍处理"，但从未走完整 `a11y-audit` 流程验证流式场景下的实际播报行为——token 级别的增量文本追加（每次 `MarkdownRender raw={item.text}` 的 `text` 变长）落在这个 live region 的监听范围内，等同于让屏幕阅读器逐字打断朗读，这是比"完全没做无障碍"更隐蔽的一类问题：静态检查一切正常（`aria-live` 属性确实存在、值确实是推荐的 `polite`），只有真正理解"这个区域在流式场景下会被触发多少次播报"才能发现问题**（Phase 6 spec 文档明确标记的"真实欠账"之一，走完整 `a11y-audit` skill 流程时核实并修复）：`specs/phases/phase-6-ai-components.spec.md` 早先如实记录了这条验收项未完成，原因写得很清楚——`index.tsrx` 里的 `aria-live="polite"` 只是"实现时的常规无障碍处理"，不是走过完整 audit 流程后确认过效果的结论。真正审计时，先用 WebSearch 查证了 AI 对话界面场景下 `aria-live` 与流式文本的行业实践共识："Do not announce every token as it streams — this creates an overwhelming flood of speech. Instead, suppress the aria-live region during streaming and announce the complete message once generation finishes."（不要逐 token 播报——会造成语音轰炸；应该在流式生成期间抑制 live region，等生成完成后播报一次完整消息）——这与当前实现（`aria-live` 从挂载到卸载全程固定为 `polite`）的差距，就是本次要修复的具体问题。**修复方式**：Foundation 层新增纯函数 `isAnyMessageStreaming(chats)`（`message.ts`，判断 `chats` 数组里是否存在 `status === 'in_progress'` 的消息——`AiMessageStatus` 类型本就有完整的 `'queued'|'in_progress'|'incomplete'|'completed'|'failed'|'cancelled'` 枚举，不需要新增状态字段），组件层用这个函数派生 `listAriaLive`：存在流式中的消息时降级为 `'off'`（不触发任何播报），全部消息完成后立即恢复 `'polite'`（此时内容已经是最终态，下一次内容变化——也就是这次状态切换本身伴随的最后一次文本更新——会被正常播报一次完整结果）。**验证方法**：CDP 的 `Accessibility` 域不直接暴露"live region 播报"这类 AT 通知事件（尝试过 `Accessibility.enable`，确认协议本身没有对应的事件订阅接口），无法用自动化脚本精确验证"屏幕阅读器实际念了几次"，因此验证分两层——(1) 用 ego-browser 真机操作 playground 里已有的流式 demo，构造"开始生成→追加增量→标记完成"三阶段的完整序列，直接读取 `aria-live` 属性在每个阶段的实际取值，确认状态机按预期转换（`polite→off→off→polite`）；(2) 落地为可重复运行的 Foundation 单测（覆盖空数组/全部 completed/存在一条 in_progress/未设置 status 四种输入）+ e2e 回归测试（对齐真机验证时走过的同一条三阶段路径）。playground demo 原本只有"开始"和"追加"两个按钮，没有"标记完成"入口，无法验证恢复 `polite` 这一半，顺手给 demo 补上了一个"标记流式完成"按钮（永久保留，不是临时验证代码）。**通用教训**：(1) `aria-live` 类无障碍属性存在"语法正确但语义失效"的风险类别——属性值本身完全符合规范推荐（`polite` 确实是持续更新内容的推荐值），但如果没有考虑"这个区域的内容变化频率/触发条件是否合理"，静态检查和常规走查都不会发现问题，必须结合组件的实际动态行为（本例是流式追加的高频次）才能判断是否真的可用，这正是 Phase SPEC 要求"走完整 audit 流程"而非"确认属性存在"的原因；(2) 排查这类"浏览器/AT 内部行为"问题时，CDP 协议不是万能的调试工具——`Accessibility` 域能查询可访问性树的静态结构，但不暴露"某个 live region 触发了几次播报"这类运行时事件，遇到协议没有覆盖的能力边界，要及时转向查证行业公开实践（本例是 WebSearch 找到的、有具体产品实践作为佐证的共识结论）作为设计依据，而不是没有实证支撑地自行假设一个"应该合理"的策略；(3) 判断某个状态机的"是否处于某种复合状态"（本例是"是否存在正在流式生成的消息"）这类跨越单条记录的整体判断逻辑，应该抽成 Foundation 层的独立纯函数（而不是内联写在组件的 `track()` 表达式里），一方面可以直接补充 Foundation 单测覆盖多种输入组合，另一方面为将来其它可能需要同一判断结果的地方（如未来给"生成中"状态加一个全局 loading 指示器）提供复用点，不要因为"目前只有一处用到"就图省事内联实现。
- **推进 Phase 4「Select/Cascader/TreeSelect 1 万节点性能基线」验收项时，核实 Select 组件本身缺少 Semi 对齐的搜索过滤能力**——spec 明确要求三者都要覆盖"面板打开、搜索输入"两个测量场景，核对 lotus Select 源码（`packages/ripple/src/input/select/index.tsrx`）发现完全没有 `filterOption`/`showSearch`/`onSearch` 之类的 prop，Cascader/TreeSelect 都有 `filterTreeNode`，唯独 Select 是空白，这不是"性能测试选一个替代场景绕过去"能解决的问题，而是一个真实的组件功能缺口，向用户报告后确认"现在就做 Select filter 功能开发"再继续。**一手来源核实**：`~/i/semi-design/packages/semi-ui/select/index.tsx` + `packages/semi-foundation/select/foundation.ts`，确认 Semi 的搜索能力由 `filter: boolean | ((inputValue, option) => boolean)` 驱动（`true` 时按 `option.label.toString().toLowerCase().includes(input)` 做大小写不敏感包含匹配，函数则完全自定义），配合 `onSearch`/`searchPosition`（`'trigger'` 默认叠加在触发器本身、`'dropdown'` 固定在面板顶部）/`searchPlaceholder`。**状态机简化**：Semi 内部维护一个独立的 `state.showInput` 字段，由 `toggle2SearchInput()` 在开关面板时显式置位/复位；核对触发路径（`_filterOption`/`open()`/`close()`）后发现这个字段本质上就是 `isOpen && filterable` 的重复表达（面板打开且开启过滤就显示搜索框，面板关闭就隐藏），照抄 Semi 的显式字段设计会引入一个完全可以用派生值替代的冗余状态，lotus 实现里改成 `let &[showTriggerInput] = track(() => !!filter && searchPosition === 'trigger' && isOpen)`，去掉了 Semi 那套需要在四五个不同关闭路径（Esc/点击外部/选中候选/close()）里都记得同步写 `showInput = false` 的心智负担——统一收口成一个 `closePanel()` 辅助函数，`filter` 开启时自动清空搜索框，避免像 Semi 那样在多处关闭出口各自决定是否重置而遗漏其中一处。**trigger 模式的渲染取舍**：Semi 的 trigger 模式用两套 CSS class（`selection-text-hide`/`selection-text-inactive`）把 input 和已选值文本同时渲染、靠 opacity 做视觉叠加动画；lotus 选择了更简单的互斥渲染（`@if (showTriggerInput) { <input> } @else if (...) { <span 已选值/placeholder> }`），面板打开时 input 和展示 span 不会同时存在于 DOM 上，牺牲了 Semi 那种"输入过程中已选值渐隐"的动画细节，换来状态更少、真机验证更容易确认没有布局重叠——这是在功能对齐之外主动做的实现简化，用 ego-browser 真机验证过 `.lotus-select-value`/`.lotus-select-placeholder`/`.lotus-select-trigger-input` 三者互斥存在，没有重叠风险。**过滤态与已选值态要分开维护两份列表**：`filteredOptions`（搜索过滤后，驱动虚拟滚动区间计算、键盘导航范围、点击选中的候选池）和 `selectedLabels`（渲染已选 tag/value 用，取自完整的 `optionList` 而非 `filteredOptions`）绝不能共用同一份数据源——已选中的选项即使被当前搜索词过滤掉，触发器上仍要能正确显示它的 label，这个功能开发时一次性设计对了没有踩坑，但在类似"父列表 vs 过滤后子列表"的场景里是个通用的检查点，值得记录。**locale 字段语义误用的连带发现**：写"无搜索结果"的空状态提示文案时，最初想照抄 Cascader 的既有写法直接复用 `locale.Select.clear`（文案是"清除"），意识到这是语义完全不对的复用（"清除"和"暂无匹配结果"是两码事，Cascader 那边这么写本身就是个遗留误用，只是不在本次任务范围内顺手修正），最终给 `SelectLocale` 新增了专门的 `emptyText` 字段（中/英文两份 locale 同步），而不是延续这个误用到新代码里——已有代码的历史误用不构成"这次可以继续这样写"的先例。
- **系统排查 Phase 4 剩余未完成验收项时，发现同一种"e2e 覆盖存在维度缺口"的模式在三个独立组件里重复出现**——ColorPicker/Slider/Rating 各自都有基础的键盘方向键测试，但都只覆盖了"最主要那一个维度"，忽略了组件自身支持的其它维度：ColorPicker 只测过 SV 面板/色相条各一个方向，没测过 SV 的垂直轴（ArrowUp/ArrowDown 控制明度）和透明度滑条（这两处键盘处理函数 `handleSvKeyDown`/`handleAlphaKeyDown` 早就实现了，只是没人在 e2e 里点过）；Slider 只测过单值模式的水平方向键+Home/End，没测过 PageUp/PageDown（10 倍步进）、`range` 模式下两个手柄各自独立的键盘操作、`vertical` 模式的键盘方向语义（Foundation 层 `handleKeyDown` 单测其实早就覆盖了这些分支，只是 e2e 没有对应验证）；Rating 只测过整数步进，没测过 `allowHalf=true` 时键盘应该按 0.5 步进（同样 Foundation 单测已覆盖，e2e 缺失）。**共性教训**：这类"组件支持 N 个维度/模式，但只写了 1 个维度的 e2e"是一种容易被忽视的欠账类型——因为跑测试会全绿（写出来的那部分测试是真实有效的），代码审查也容易漏看（每个键盘处理函数单独看都完整），只有对照"这个组件实际暴露了哪些 prop/模式组合"逐项复核才能发现。排查方法：不要只看"有没有键盘测试"，要看"这个组件的每一种模式/prop 组合是否都至少被测过一次"——尤其是 `range`/`vertical`/`allowHalf` 这类改变行为语义的 boolean prop，開啟後常常会解锁一整条从未被验证过的代码路径。同一次排查还发现 Cascader/TreeSelect 的"级联勾选"测试只覆盖了勾选方向（父→子）和叶子节点自身取消，缺失"取消勾选父节点后子孙同步取消"这一半对称场景——级联类操作作为一个整体，"做"和"撤销"是两个独立方向，只测一个方向不能代表整个行为被验证过。
- **`specs/phases/phase-4-complex-data-entry.spec.md` 第 26 行「`virtualList.ts` 需覆盖条目高度不固定场景」这条验收标准的字面要求与既定架构决策直接冲突**——`packages/foundation/src/base/virtual-list.ts` 的实现从一开始就是固定行高设计（`itemHeight: number` 单一数值，不是按索引取值的函数），源码注释里写明这是"对齐 Semi react-window `FixedSizeList` 与 chenzy.design 参考实现两方都验证过的合理最小范围"，是经过调研的主动选择，不是遗漏。遇到这种"spec 字面要求 vs 已经落地且有理有据的架构决策"冲突时，正确做法不是硬造一个测不出来的测试（可变行高场景在当前实现下根本不存在，无法针对一个不存在的能力写单测），而是回头如实修正 spec 文字，把原因写清楚（源码注释在哪、决策依据是什么），让 spec 与代码重新对齐——这也是"以核对源码为荣，以轻信文档为耻"在验收标准本身出现偏差时的应用：不仅组件实现要核对源码，spec 里的验收标准本身也可能是过时或写错的，同样需要核对。
- **Table 的列级固定列（`column.fixed`）是本次排查发现的最典型"Foundation 写好了，组件层从未接上"半成品案例**——`packages/foundation/src/show/table/fixed-column.ts` 的 `calcFixedOffsets`/`hasFixedColumns` 早就存在且有 7 条独立单测，函数注释里甚至写明"真实宽度来自渲染后测量的 DOM，由 .tsrx 组件层采样后传入 widths 数组"这种明确期待组件层接入的设计意图，但 `packages/ripple/src/show/table/index.tsrx` 从头到尾没有一处 `import` 这个模块——`grep calcFixedOffsets index.tsrx` 直接返回空。更隐蔽的是配套的 e2e 测试给出了错误的信心：测试标题叫"固定列：左右固定列渲染 sticky 定位样式"，读起来像是验证过完整功能，但断言体只检查了 `thead th` 的文字内容，从未读取过任何 `getComputedStyle`/`position`/`left`/`right`。这种"标题描述了功能、断言只测了皮毛"的测试比完全没有测试更危险——完全没测试至少会让人心虚去手动检查，一条名不副实但绿色的测试会给出虚假的完成感。**发现路径**：不是靠读组件代码逐行找出来的，而是在推进"Phase 4 spec 要求 Table 按里程碑独立验收记录"这个文档整理任务时，习惯性地对每个里程碑去 `grep` 对应 Foundation 模块名在组件层的引用情况，才发现这个从未连接的接口。**教训**：（1）"写验收记录/整理文档"这类看似纯文字工作，做的时候如果对每一条都去核实真实代码引用而不是照抄已有记录，能筛出被之前几轮开发遗漏的真实功能缺口——本条踩坑正是"以核对源码为荣，以轻信文档为耻"原则应用到"整理已有 spec 记录"这个具体场景的例子；（2）Foundation 层单测覆盖率高不代表功能完整——纯函数本身测得再充分，如果组件层没有调用它，用户拿到的实际效果是零，"Foundation 单测通过"和"功能已交付"是两件完全独立的事，必须额外核实调用链路的存在性；（3）**修复方案的核心决策**：偏移量必须用 DOM 实测宽度而非 `column.width` 声明值（对齐 Semi `arrayAdd(headWidths, ...)` 的做法）——因为实际渲染宽度可能因 padding、内容溢出、用户拖拽调整过列宽（`resize-column.ts` 的 `state.columnWidths`）而与声明值不同，用声明值算出的偏移量在这些场景下会错位；测量时机用 `effect` 依赖 `leafColumns`（列结构变化）和 `state.columnWidths`（拖拽后）触发 `queueMicrotask` 重新测量，不用 `ResizeObserver` 逐列监听（`<th>` 尺寸变化不受固定列计算结果影响，理论上观察它不会形成响应式循环，但为一个不需要连续监听的场景维护 N 个 observer 实例是不必要的复杂度）；（4）真机验证这类"视觉定位是否生效"的功能时，直接读取 `getComputedStyle` 的 `position`/`left`/`right` 数值，比截图更适合自动化验证——截图需要人眼判断，数值断言可以精确对比滚动前后是否真的保持了 0px 位置不变，配合 `--repeat-each` 验证时序稳定性（本例中 DOM 测量依赖 `queueMicrotask`，8/8 稳定通过排除了竞态风险）。
- **核实 Phase 1 spec「FloatButton 的 AI 主题变体视觉通过对比度检查」这条时，从一个简单的对比度计算任务，一路挖出 `aiColor` 整个模块与 Semi 一手来源完全脱节的问题**——最初只是想算 `--lotus-color-ai-general` 背景上白字的 WCAG 对比度，结果发现这个渐变本身的角度（90deg vs Semi 一手来源 278deg）、色标数量（3 个 vs 4 个）、颜色顺序（紫→粉 vs 粉紫→蓝）全都对不上；继续核实 `purpleHover`/`purpleActive` 发现是用 `withAlpha()` 给同一个基准色降透明度模拟出来的，但 Semi 一手来源里这两个状态是完全独立的色值（`ai-purple-6`/`ai-purple-7`，不是 `ai-purple-5` 的透明度变体）；再核实 `backgroundTop`/`backgroundBottom` 发现 Semi 原本是复杂的多色标渐变（亮色 4 色标 201.15deg，暗色又是不同的 3 色标组合），lotus 只用了一个简单的单色半透明 rgba 近似替代。**发现路径的教训**：像"检查一个组件的对比度是否达标"这种听起来范围很窄的验收项，如果第一步就去核对真正驱动视觉的 token 定义是否对齐一手来源（而不是直接拿现有实现的色值去套公式），经常会在核对过程中意外揭出比原始任务大得多的问题——这次的连锁反应是从 1 个组件的 1 条验收标准，扩大到修正整个 `aiColor` 模块（影响 FloatButton/Button/Tag 三个消费方）。**修复原则**：渐变色不能靠"看起来差不多"的近似值替代，必须把一手来源的每一个色标（`_palette.scss` 里 `-0`/`-1`/`-2`/`-3` 这种编号）逐个转成 hex 后原样拼回 `linear-gradient()` 字符串，角度、色标位置百分比（`0%`/`30%`/`60%`/`100%`）、颜色顺序都要精确对齐，任何一处"觉得差不多所以简化了"都会导致后续用公式算出来的对比度、以及最终呈现给用户的视觉效果，与真正的设计意图产生偏差。发现的巧合佐证：`packages/icons` 的 `MULTI_COLOR_ASSETS` 注释里其实早就记录了同一套一手来源渐变色（`#E945FF → #A647FF → #6B61FF → #2E8CFF`，用于 AI 等级图标），只是这个已经验证过的正确色值从未被同步进 `static-tokens.ts` 的 `aiColor` 模块——同一个仓库里，一个模块做对了、另一个模块做错了，说明"局部正确"不能代替"全局核对"，遇到看起来相关的两处实现，值得交叉检查它们是否本该共享同一份一手来源数据。**对比度结论**：修正后 default 态最低对比度从错误实现的 2.83:1 提升到 3.11:1，仍略低于正文 4.5:1 但满足 UI 组件 3.0:1 门槛，这是 Semi 官方"多彩渐变+统一白字"设计范式本身的固有权衡（Semi 自己在生产环境也是这样），记入「已知设计权衡」而非当作缺陷继续修（对齐此前 Success/Warning/Danger 按钮白字对比度那条已有先例的处理方式）——同时也验证了"先核对一手来源、再判断是否权衡"的必要性：如果不核对一手来源就直接对当前实现的 2.83:1 下结论，可能会误以为"这就是 Semi 的固有权衡"，但其实那个数字本身就是算在错误颜色上得出的错误结论。
- **单文件多组件模式下，即使 `<style>` 块正确写在了实际渲染该 class 元素的那个组件函数自己内部（不是踩坑 #68 那种"写错了函数"），如果该组件函数体的 JSX 顶层根节点直接是一个真实 DOM 元素（`<div>`/`<li>` 等）而不是 `<>` Fragment，这个根节点元素本身可能不会被打上 scoped CSS 所需的 `tsrx-xxxxxxxx` class，导致选择器 `.classname.tsrx-xxxxxxxx` 不匹配、这条规则完全不生效——而该组件内部通过 `@for`/`@if` 渲染的子元素通常能正常拿到 scope class（因为它们经过了独立的渲染路径），所以现象是"同一个组件内，子元素样式正常、顶层根元素自己的样式却像没生效"**（踩坑 #82，重大，Calendar 组件排查发现，与踩坑 #68 同族但触发条件更具体）：本例中 `CalendarMonthWeekRow`/`CalendarDayEvents` 两个顶层组件函数都是 `<div class="...">...</div>` 直接作为返回值（`<style>` 紧跟在 div 内部，本身没有位置问题），`getComputedStyle()` 显示 `.lotus-calendar-month-week-row` 的 `min-height: 96px` 规则完全不生效、`row.className` 打印出来只有裸类名没有 `tsrx-xxxxxxxx` 后缀，而同一文件里所有顶层是 `<>...</>` 的组件（`CalendarDaysView`/`CalendarMonthView`）scope class 都正常附着。这个 bug 长期潜伏未被发现的原因：`.lotus-calendar-month-week-row` 的 `min-height` 只在"容器高度小于内容自然高度"时才有视觉差异，此前月视图从不限制显示的事件数量，事件全部用绝对定位铺满，容器实际高度被内容撑起来看起来"正常"，直到本次新增 `itemLimit`（根据格子实测高度换算能显示几行事件）功能后，`getBoundingClientRect().height` 直接读到了错误的、被压缩的高度值（36px 而不是 96px），才第一次真正暴露。**排查路径**：先怀疑测量时机（ResizeObserver 首次回调是否在布局完成前触发），排除后逐层用 `getComputedStyle` 核对该元素的每条自身 CSS 规则是否生效，发现 `min-height` 的 computed 值是 `auto` 而不是声明的 `96px`，进而检查 `element.className`/`element.matches(selector)` 才发现根节点缺失 scope class。**修复方式**：把两个组件的 JSX 顶层从 `<div class="...">...</div>` 改成 `<>\n  <div class="...">...</div>\n  <style>...</style>\n</>`（`<style>` 移到 div 外部、Fragment 内部），scope class 立即正确附着。**通用教训**：任何顶层函数组件如果直接 `return`（或 tsrx 里对应的隐式渲染）一个真实 DOM 元素而不是 `<>` Fragment，都要额外核实这个根元素自身是否拿到了 scope class（`element.className` 打印出来是否含 `tsrx-` 前缀的 token，或直接 `getComputedStyle` 核对该组件自己 `<style>` 里写的规则是否生效）——不能仅凭"子元素渲染正常、DOM 结构符合预期"就认为整个组件的样式都没问题，顶层根节点是这条规则的隐藏例外。同一文件内新增组件时，一律让顶层用 `<>...</>` 包裹（即使当前只有一个根元素），不要图省事直接返回单个 DOM 节点，从源头规避这类 scope 丢失。

- **一个组件的 `<style>` 里写选择器去样式化"由另一个组件渲染、作为 children 传入的元素"（如 `.lotus-tag-split .lotus-tag {...}`，`.lotus-tag` 是 Tag 组件自己的 class，不是 SplitTagGroup 声明的），这条规则会被 tsrx 编译器直接从最终 CSS 里整条剔除（不是选择器优先级问题、也不是没匹配到元素，是编译产物里根本找不到这条规则）——正确写法是用 `:global(.classname)` 包裹这个"外来" class 名，如 `.lotus-tag-split > :global(.lotus-tag) {...}`**（踩坑 #68 续，完整解决方案，SplitTagGroup 组件开发发现，`CardGroup` 的 `.lotus-card-group-grid > :global(.lotus-card)` 是已验证过的先例）：现象和踩坑 #68/#82 很像（DOM 结构、class 名都对，样式就是不生效），但排查路径不同——先用 `document.styleSheets` 遍历确认这条规则是否真的出现在最终 CSS 里（`sheet.cssRules` 里搜索关键字），如果完全找不到这条规则（而不是规则存在但选择器不匹配、或者存在但被更高优先级覆盖），就是这个"跨组件选择器被裁剪"的场景，改用 `:global()` 包裹目标 class。**适用场景判断**：只要一个组件的 CSS 需要样式化"从 props/children 传入、但由另一个组件内部渲染出来的元素"（子元素的 class 名不是当前组件自己在 JSX 里声明的），就要用 `:global()`，不能假设普通嵌套选择器会生效——这跟"两个组件各自的 `<style>` 互相看不到对方 class"（踩坑 #68 原始场景）是同一个底层机制的两种表现形式，一个是"完全没在当前组件出现过的 class"会被裁剪，一个是"当前组件声明了但另一个组件的规则引用它"不生效，本质都是 tsrx scoped CSS 只认"当前组件 JSX 里实际写出来的 class 名"。

- **Modal 完全没有接入 body 滚动锁定，SideSheet 有正确的引用计数实现但是模块私有、与 Modal 各自为政**——排查 Phase 3「多个模态嵌套打开/关闭时锁定状态计数正确」这条验收标准时发现。`packages/ripple/src/show/modal/index.tsrx` 从头到尾没有任何 `document.body.style.overflow` 操作，Modal 打开期间背景内容仍然可以滚动；`side-sheet/index.tsrx` 里倒是有一套完整且设计正确的引用计数（`scrollLockCount`/`originalBodyOverflow` 模块级变量 + `lockBodyScroll`/`unlockBodyScroll`），但这套逻辑是 SideSheet 文件私有的，从未导出给其他组件共用。**这不是"SideSheet 的计数器设计有 bug"（子代理最初的判断），而是"两个组件各自为政、互不知晓对方状态"**：如果只是 SideSheet 自己开关多次，引用计数完全正确；真正的问题场景是"Modal 打开 → SideSheet 也打开 → SideSheet 关闭"，此时 Modal 因为完全没有调用锁定逻辑，不会对同一个计数器产生任何影响，看起来像是"只要 Modal 参与就会出问题"，但根因是 Modal 一开始就没接入，不是计数逻辑本身算错了。**修复方式**：把 SideSheet 私有的锁定逻辑提取成 `packages/foundation/src/base/scroll-lock.ts` 共享模块（与 `focus-trap.ts` 同一组织模式——Modal/SideSheet 已经共享了焦点陷阱的纯计算函数，这次补上滚动锁定），两个组件都改为 `import { lockBodyScroll, unlockBodyScroll }`，共用同一个模块级计数器。**一手来源核对的意外发现**：核对 Semi `semi-ui/modal/Modal.tsx` 时确认 Modal 官方实现是 `disabledBodyScroll`/`enabledBodyScroll` 两个内部方法，跟随打开/关闭生命周期无条件执行，完全没有暴露可关闭的 prop；而 Semi SideSheet 却真的有 `disableScroll` 这个可配置 prop（`semi-ui/sideSheet/index.tsx` 确认）——两个组件在 Semi 官方就存在这层 API 差异，不是 lotus 自己引入的不一致，修复 Modal 时不需要画蛇添足地也加一个 `disableScroll` prop 去追求"两个组件 API 对称"，遇到"看起来应该对称但一手来源本来就不对称"的情况，应该以一手来源为准而不是主观追求表面一致性。**测试环境的教训**：Foundation 层给 `scroll-lock.ts` 写单测时发现这个包的 vitest 默认运行在 Node 环境（`typeof document === 'undefined'`），不是 jsdom——凡是要测试真实操作 `document.body`/`window` 的模块，都要用 `vi.stubGlobal('document', mockObject)` 手动构造最小 mock 对象（`{ body: { style: {...} }, documentElement: { clientWidth } }`），不能假设浏览器全局对象天然可用；e2e 层面验证"Modal 打开时 SideSheet 也打开"这种跨 Portal 组合场景时，Modal 的遮罩层会按设计正确拦截指针事件、挡住页面上其它按钮的点击——这是真实的、正确的 UI 行为不是 bug，但会让 Playwright 的 `locator.click()` 因为"目标被遮挡"而重试超时，这类测试只关心底层状态机逻辑（不关心真实用户能否点到）时，改用 `page.evaluate(() => el.click())` 直接在 DOM 层触发点击事件绕过指针事件拦截层，是比强行摆脱遮罩布局更实际的做法。
