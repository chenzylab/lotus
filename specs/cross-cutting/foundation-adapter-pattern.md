...
- **Popconfirm（`trigger="custom"` 完全自控开关状态）的确认/取消回调在非受控模式下直接调用 `Foundation.setState({ visible: false })` 改内部 state，不经过组件层统一的 `setVisible()` 函数，导致 `onVisibleChange` 回调在“确认/取消关闭”这条路径上完全不会被调用（只有点击外部/Esc 走 Popover 自身的 `onVisibleChange` 转发才会通知外部）——这是排查焦点归还问题时顺带发现的独立缺陷，未在本次任务范围内修复，留痕待后续处理**：`packages/foundation/src/feedback/popconfirm/foundation.ts` 的 `handleConfirm`/`handleCancel` 同步分支直接 `this.setState({ visible: false })`，这只在非受控模式下驱动 `state.visible` 进而驱动 `displayVisible`/`Popover.visible` 变化使浮层视觉关闭，完全绕开了 `packages/ripple/src/feedback/popconfirm/index.tsrx` 里 `setVisible()` 函数内对 `onVisibleChange?.(next)` 的调用——父组件监听 `onVisibleChange` 想在“用户确认/取消后”做一些收尾（如埋点、聚焦转移）时，非受控模式下永远收不到这个通知，只有受控模式（父组件自己传 `visible` prop）才会因为父组件自己改 `visible` 而间接触发。另外发现 Popconfirm 因为固定用 `trigger="custom"`（`isControlled` 恒为 `true`），继承的 Popover Esc 关闭逻辑里有 `!isControlled` 判断（受控模式下 Popover 不会擅自处理 Esc，因为可见性真源在外部），导致 Popconfirm 目前**没有任何路径支持 Esc 键关闭**——这两个问题都不是本次"焦点归还"改动引入的回归（本次新增的焦点归还监听的是 `displayVisible` 本身的值变化，不依赖 `setVisible` 是否被调用，因此确认/取消路径的焦点归还本身是正确生效的，已真机验证），但反映出 Popconfirm 的事件通知链路和 Esc 支持存在真实缺口，需要专门的后续任务处理（可能需要重新设计 `handleConfirm`/`handleCancel` 让它们统一走一个"关闭"出口，并评估 Popconfirm 是否应该像 Semi 一样自行处理 Esc 而不是依赖继承 Popover 的受控判断）。
...
- **已知问题待修复（非本次改动引入，Select 组件虚拟化改造真机验证时偶发暴露）**：`packages/ripple/src/input/select/index.tsrx` 第338行 `@if (selectedLabels.length > 0) { <span>{selectedLabels[0]!.label}</span> }`，`length > 0` 判断和 `[0]!` 非空断言读的可能是两个不同时刻的 `selectedLabels` 值（响应式派生值在渲染期间被重新计算，两次读取之间发生了变化），并发跑多个 e2e 测试文件时命中过一次 `Cannot read properties of undefined (reading 'label')`，但没有导致任何测试失败（只是控制台一条未捕获错误 noise），复现条件不稳定，未深入排查根因。**待办**：后续排查时优先怀疑"`@if` 条件表达式和分支体内表达式各自独立求值、非同一次快照"这个方向（同类模式在别的组件也可能存在，比如别处出现的 `arr.length > 0` 后紧跟 `arr[0]` 的写法都值得抽查），修复思路大概率是把 `selectedLabels[0]` 先赋值给一个局部变量/派生 track 再在两处复用同一个值，而不是在 `@if` 条件和分支体里分别重新读取响应式表达式。
