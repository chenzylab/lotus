...
> 核实说明（未到发布节点时的状态）：这份清单是**每次实际执行发布动作前**要走的流程性检查，不是"代码库现状是否具备某能力"这类静态验收项——核实时发现 `changesets` 工具尚未接入本仓库（无 `.changeset` 目录、根 `package.json` 无相关脚本），各包版本号仍是初始值 `0.0.0`（如 `@lotus/ripple`），这是因为目前 Phase 2/5 仍在收尾、尚未到"Phase 性 SPEC 全部完成"这个发布触发点（对照第 5 条），不是遗漏或缺陷，暂不勾选也不视为真实欠账。以下几条会在第一次真正执行发布时逐条走一遍再勾选：
