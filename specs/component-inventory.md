...
- [x] Table 表格（含虚拟化/树形数据）（Semi 源码 + chenzy.design 双重调研核实；用户明确选择三期能力一次性交付，不做分期后置；数据管线（排序 → 筛选 → 分页 → 打平）与行选择/展开/树形三态级联全部下沉 Foundation 纯函数模块（table-data/sort/filter/row-selection/expand/fixed-column/resize-column），组件层只做 DOM 渲染与事件转发；行选择的父子级联复用 Tree 组件已验证的 check-cascade 算法（buildKeyEntities/calcCheckedKeys），不重新发明；多级表头用 buildHeaderRows 把 columns.children 摊平成行×格网格，叶子列 rowSpan 补齐到表头总行数；方向键单元格导航明确不做——Semi 和 chenzy.design 两方均验证过判定不需要，维持原生 Tab 序；aria-label 挂在最外层 wrapper 而非 `<table>` 本身，便于测试和外部脚本定位整个 Table 区域。全面验收审计后补充：`virtualized?: boolean`/`rowHeight?: number` 两个字段此前从未被组件体消费是死 prop，改造为真实生效的 `virtualize?: { itemSize: number }`——调研 Semi 源码确认其虚拟滚动用 react-window 的 VariableSizeList 整体替换 tbody，固定列靠 CSS `position:sticky` 与滚动内容同属一行 `<tr>` 天然对齐、不需要独立同步机制，但 lotus 当前 Table 的列 `fixed` 配置本身也是从未实现的死 prop（`e2e/show/table.spec.ts` 原有的"固定列"测试用例名不副实，实际只断言了表头文字，未验证任何 sticky 样式），本次不需要处理固定列共存；虚拟化仅在同时声明 `scroll.y`（固定容器高度，此前也从未被消费）且没有 `expandedRowRender`（展开行内容任意高度，与固定行高假设冲突）时生效；`flatRows` 复用 Foundation 已摊平好树形展开态的定长数组（与 TreeSelect 的 flatNodes 同一模式）；HTML `<table>` 的 `<tbody>` 不能像 Select/TreeSelect 那样嵌一层 `translateY` 容器，改用两个高度分别为 offsetY / 剩余总高度的占位 `<tr>` 撑开滚动条长度；单行渲染抽出 `TableRow` 子组件供虚拟化/非虚拟化两条路径共用，按踩坑 #97 的教训给子组件补上独立 `<style>` 块，其中 `.lotus-table-cell` 等表头（`<thead>`，主组件直接渲染）与数据行（`<tbody>`，`TableRow` 子组件渲染）共用的 class 需要在主组件和子组件两侧的 `<style>` 块内各自完整定义一份；e2e 覆盖新增 1 用例验证 1 万行数据只渲染可见区间+滚动动态切换+勾选状态不受虚拟化裁剪影响（勾选态存于 `state.selectedRowKeys` 全量 key 集合，不因虚拟化裁剪丢失），`--repeat-each=10` 10/10 全过，`e2e/show/table.spec.ts` 全量 11/11 通过）
